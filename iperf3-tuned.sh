...
read_config() {
    ...
    SUDO_PASS=$(jq -r '.sudo_pass // ""' "$CONFIG_FILE")
    SUDO_PASS_FILE=$(jq -r '.sudo_pass_file // ""' "$CONFIG_FILE")
}
...
cmd_init() {
    ...
    local ssh_host ssh_user ssh_key ssh_pass sudo_pass auth_choice
    ...
    if [[ $INTERACTIVE -eq 1 ]]; then
        ...
        if [[ -n "$ssh_host" ]]; then
            ...
            if [[ "$auth_choice" == "2" ]]; then
                ...
                echo
                read -rsp "  sudo 密码 (留空=与 SSH 密码相同): " sudo_pass
                echo
            fi
        fi
    else
        ...
        ssh_key="$SSH_KEY"; ssh_pass="$SSH_PASS"; sudo_pass="$SUDO_PASS"
    fi
    ...
    jq -n \
        ...
        --arg sudo_pass "$sudo_pass" --arg sudo_pass_file "" \
        --argjson ssh_port 22 \
        '{server:$server, port:$port, duration:$duration, repeats:$repeats,
          max_parallel:$max_parallel, profile:$profile,
          ssh_host:$ssh_host, ssh_user:$ssh_user,
          ssh_key:$ssh_key, ssh_pass:$ssh_pass, sudo_pass:$sudo_pass, sudo_pass_file:$sudo_pass_file, ssh_port:$ssh_port}' \
        > "$CONFIG_FILE"
