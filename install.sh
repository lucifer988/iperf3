...
install_with_apt() {
    ...
    apt-get install -y -q --no-install-recommends \
        jq iperf3 ethtool iproute2 sysstat bc tar iputils-ping util-linux
}
install_with_dnf() {
    dnf install -y -q epel-release 2>/dev/null || true
    dnf install -y -q jq iperf3 ethtool iproute sysstat bc tar iputils util-linux
}
install_with_yum() {
    yum install -y -q epel-release 2>/dev/null || true
    yum install -y -q jq iperf3 ethtool iproute sysstat bc tar iputils util-linux
}
install_with_pacman() {
    pacman -Sy --noconfirm --needed jq iperf3 ethtool iproute2 sysstat bc tar iputils util-linux
}
install_with_apk() {
    apk update >/dev/null 2>&1 || true
    apk add --no-cache jq iperf3 ethtool iproute2 sysstat bc tar iputils-ping util-linux
}
install_with_zypper() {
    zypper --non-interactive refresh >/dev/null 2>&1 || true
    zypper --non-interactive install --no-recommends \
        jq iperf3 ethtool iproute2 sysstat bc tar iputils util-linux
}
...
if "${INSTALL_DIR}/iperf3-tune" --help >/dev/null 2>&1; then
    _installed_ver=$("${INSTALL_DIR}/iperf3-tune" --version 2>/dev/null || echo "?")
    echo "  iperf3-tune: OK (已安装版本 ${_installed_ver}, 路径 ${INSTALL_DIR}/iperf3-tune)"
    # v2.9.2: 自检关键修复是否生效 (不需要 iperf3/网络/root)
    if "${INSTALL_DIR}/iperf3-tune" selftest >/dev/null 2>&1; then
        echo "  selftest:   通过 (关键修复均生效)"
    else
        echo "  selftest:   ${C_YELLOW}有项未通过, 运行 'iperf3-tune selftest' 查看${C_RESET}"
    fi
    # 若 PATH 里还有别的 iperf3-tune, 提醒避免跑到旧的
    _which=$(command -v iperf3-tune 2>/dev/null || echo "")
    if [[ -n "$_which" && "$_which" != "${INSTALL_DIR}/iperf3-tune" ]]; then
        echo "  ${C_YELLOW}注意: PATH 中 'iperf3-tune' 实际指向 ${_which}, 可能是旧版本!${C_RESET}"
        echo "  ${C_YELLOW}      请用全路径 ${INSTALL_DIR}/iperf3-tune, 或调整 PATH 顺序${C_RESET}"
    fi
else
...
${C_BOLD}v3.0.0 新特性（NIC 目标反馈闭环）：${C_RESET}
  ★ --scan* 先单流参数寻优，未达到目标利用率自动进入多流寻优
  ★ 默认 NIC 目标为 85%，可用 --target-utilization 调整；state.json 记录停止原因

${C_BOLD}v2.7.0 新特性 (生产安全加固):${C_RESET}
  ★ 默认不再自动装依赖 (缺啥提示你装; 需要时显式 --auto-install)
  ★ --dry-run 预演: 只打印将修改的内容, 完全不动系统
  ★ 改系统前列影响清单 + 二次确认; 无 tty 不再静默执行 (需 --yes)
  ★ SSH 不可达 / 无主网卡 时优雅降级, 不再中途崩溃
  ★ 密码安全: 推荐 --ssh-key / --ssh-ask-pass; --detach 不再在 ps 暴露明文
v2.6.0 新特性 (双端 CPU 寻优 + 零拷贝发送端感知):
  ★ --scan-cpu 两端都扫 (iperf3 -A n/m 同时绑本端与远端 server 核)
  ★ -Z 零拷贝: -R 下经协商作用在 server, 并核实 server iperf3 版本
v2.5.0 新特性 (单流自动寻优):
  ★ --scan 自动扫描 拥塞算法+窗口+CPU, 选最优组合
  ★ -R 方向感知: server 是发送端, 拥塞扫描自动作用在 server
v2.4.0 新特性 (单流专项):
  ★ --single 仅测单流; 自动绑核 (NUMA 对齐) + 按 BDP 自动算窗口 + 零拷贝
  ★ --mtu 9000 一键 jumbo frame (本机 + 远端同步)
  ★ tune 时停 irqbalance 保持 IRQ 亲和; rollback 自动恢复
EOF
