...
- **改进寻优早停逻辑**。v2.1 单流重传率超阈值就立刻终止寻优,但多流的意义恰恰是分散丢包(单流 5% 重传,4 流可能只 0.5%)。现在:单流不允许早停(除非离谱 > 10%),从第二档开始才允许。
- **默认重传率阈值 1.0% → 3.0%**。1% 仅适合裸金属/同机房;公网/跨地域链路上 1-3% 重传是 BBR 设计内的正常表现。
- **新增链路诊断分析 (`analyze_link`)**。bench 完成后自动测 RTT、推断链路类型(本机/LAN/同区域/跨地域/跨境)、给出**针对该类型的**重传率评级和明确的"该不该继续调"建议。
- **明确告诉用户"调优是否成功"**。之前结尾只显示"结果已保存",让人不知道是不是搞完了。现在最后一行明确写 `本次调优已生效 ✓ / sysctl 持久化在 ... / 想回退...`。
- **会识别"NIC 10G 但只跑出 ~1Gbps"这种 ISP 限速场景**,提示用户排查中间链路(`mtr`)而不是继续调 sysctl。
- `state.json` 里加 `rtt_ms` / `loss_pct` 字段。

**v2.1** —— 修 bug: 重传率显示永远 0.000% (iperf3 TCP 模式无 packets 字段,改用 bytes/MSS 估算);输出加上原始重传次数;`--mss N` 参数;`-v` 模式保留每次 iperf3 原始 JSON 便于核对

**v2.0** —— 后台运行 (`--detach`)、进度增量保存、`watch`/`tail`/`stop` 子命令、SSH 密码 (`--ssh-pass` / `--ssh-pass-file`) 走 SSHPASS env、ethtool 解析改用段头定位、多档中位数寻优、评分函数 `bw × (1 − k × retrans_rate)`
