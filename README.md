...
- **随时可回退**：`iperf3-tune rollback` 会还原 sysctl、恢复 `irqbalance`、还原 MTU。部分 offload 改动不可逆，最干净的还原是「移除配置文件 + 重启」。
...
回退会：还原备份的 sysctl、删除 `/etc/sysctl.d/99-iperf3-tune.conf`、还原 MTU、恢复 `irqbalance`。部分网卡 offload 改动（如 LRO）不可逆，提示中会说明——最干净的还原是移除配置文件后重启。
