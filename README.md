...
- **修 bug: 重传率始终显示 0.000%**。v2.0 用 `.end.streams[].sender.packets` 算分母,但 iperf3 TCP 模式根本不输出 packets 字段(只 UDP 模式有),分母永远 0 → 百分比永远 0%。改用 `bytes / MSS` 估算总段数。
- **输出加上原始重传次数**。之前只显示百分比,现在 `重传 1234 次 (0.123%)` 这样,即使 MSS 估算有偏差也能直接看绝对值。
- 新增 `--mss N` 参数。默认 1448 适用于 1500 MTU 千兆链路;jumbo frame (MTU 9000) 改 8948。
- `--verbose` 模式下,每次 iperf3 的原始 JSON 输出会保留在 `/var/log/iperf3-tune/last-iperf3-*.json`,便于核对。
- `state.json` / `progress.json` 里新增 `retransmits`、`est_segments`、`bytes_sent` 字段。
