...
这个模式最适合你，因为只需要回答提示问题。交互模式跑完后，会明确再问一次：仅回滚、保留当前最优运行态，还是直接持久化最佳配置。
...
脚本现在会**跳过这个 profile，继续跑剩余 profile**，不会因为单个失败直接让整轮 `auto-all` 报废。

汇总里也会保留失败项，方便你回头看是哪一档 profile 出问题。
