...
`--scan` 现在是目标反馈闭环：先按“拥塞算法 → TCP 窗口 → CPU”完成单流寻优；如果单流未达到 NIC 标称速率的目标利用率（默认 85%），会自动继续测试 `-P 2/4/8/16/32`，达到目标或评分/重传率表明继续无收益时停止。`--single` 仍严格只测单流。

扫描逻辑大致是：

```text
单流参数寻优 -> NIC 利用率判断 -> 多流并发寻优 -> 达标/无收益停止
```
...
6. 测试结束后自动回收“本次脚本临时拉起”的远端 iperf3 -s
```
...
- 如果远端 iperf3 -s 原本就在跑，脚本不会关掉它；只会回收本次自己临时拉起的进程
```
...
| `--target-utilization PCT` | scan 闭环目标 NIC 利用率，默认 85% |
