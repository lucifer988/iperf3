...
choose_profile() {
  ...
  # 优先使用 BBR v3 > BBR v2 > BBR v1
  if grep -qw bbr3 <<<"$available_cc"; then
    echo "[远程] 检测到 BBR v3，优先使用" >&2
    printf '%s\n' 'bbr3-fq'
  elif grep -qw bbr2 <<<"$available_cc"; then
    echo "[远程] 检测到 BBR v2，优先使用" >&2
    printf '%s\n' 'bbr2-fq'
  elif grep -qw bbr <<<"$available_cc"; then
    echo "[远程] 检测到 BBR v1" >&2
    printf '%s\n' 'bbr-fq'
  else
    echo "[远程] 未检测到 BBR，使用 cubic + fq_codel" >&2
    printf '%s\n' 'cubic-fq_codel'
  fi
}
...
apply_profile() {
  ...
  case "$profile" in
    bbr3-fq) cc=bbr3; qdisc=fq ;;
    bbr2-fq) cc=bbr2; qdisc=fq ;;
  ...
  if [[ -n "$iface" ]]; then
    # 针对 fq 启用 pacing
    if [[ "$qdisc" == "fq" ]]; then
      tc qdisc replace dev "$iface" root fq pacing >/dev/null 2>&1 || true
    else
...
sysctl -w net.ipv4.tcp_rmem="4096 87380 $WMEM_MAX" >/dev/null
...
if [[ "$REMOTE_PERSIST" == "1" ]]; then
  cat > /etc/sysctl.d/99-iperf3-remote-tune.conf <<EOF
# Generated by iperf3-easy.sh remote tuning
# Server-side send optimization for iperf3 -R mode

# 基础缓冲区
net.core.rmem_max=$WMEM_MAX
net.core.wmem_max=$WMEM_MAX
net.ipv4.tcp_rmem=4096 87380 $WMEM_MAX
...
choose_local_sender_lists() {
    ...
    if [[ "$SWEEP_LOCAL_SENDER" -eq 1 ]]; then
        # 优先检测 BBR v2/v3，然后是 BBR v1
        if grep -qw bbr3 <<<"$available_cc"; then
            log "检测到 BBR v3，优先使用"
            CC_LIST=(bbr3 bbr cubic)
        elif grep -qw bbr2 <<<"$available_cc"; then
            log "检测到 BBR v2，优先使用"
            CC_LIST=(bbr2 bbr cubic)
        elif grep -qw bbr <<<"$available_cc"; then
            log "检测到 BBR v1"
            CC_LIST=(bbr cubic)
        else
...
apply_local_sender_profile() {
    ...
    if [[ -n "$iface" ]]; then
        # 针对 fq 队列优化参数
        if [[ "$qdisc" == "fq" ]]; then
            if tc qdisc replace dev "$iface" root fq pacing >/dev/null 2>&1; then
                CURRENT_QDISC_CHANGED=1
            else
                warn "tc qdisc replace dev $iface root fq 失败（非致命）"
            fi
        else
...
# 重传惩罚：按重传率而非绝对值，避免高带宽场景误判
# sender_retrans 是服务端发送侧重传（-R 模式下最关键）
if sender >= 0:
    # 基础惩罚 + 非线性惩罚（重传越多，惩罚越重）
    retrans_rate = sender / max(mbps * 60 / 8 / 1500, 1)  # 估算重传率
    if retrans_rate > 0.01:  # 超过 1% 重传率，额外惩罚
        penalty += sender * 50.0
    else:
        penalty += sender * 25.0
