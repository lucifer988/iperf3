#!/usr/bin/env bash
#
# iperf3-tune.sh — iperf3 高速低重传优化工具 (v2.9.4)
#
...
cmd_selftest() {
    ...
    _t "set_sender_congestion 含 return 1" \
       "$(declare -f set_sender_congestion | grep -c 'return 1')" "2"
    ...
    _t "密码模式强制 password 认证" \
       "$(declare -f _ssh_prefix | grep -c 'PreferredAuthentications=password')" "1"
