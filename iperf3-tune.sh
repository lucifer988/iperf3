#!/usr/bin/env bash
#
# iperf3-tune.sh — iperf3 高速低重传优化工具 (v2.9.2)
#
...
WINDOW_MAX="32M"           # v2.9.2: 自动窗口绝对上限降到 32M (≤2G 链路足够; 低内存机更安全)
...
clamp_window_bytes() {
    ...
    cap_abs=$(to_bytes "$WINDOW_MAX"); (( cap_abs > 0 )) || cap_abs=$((32*1048576))
    cap_kern=$(kernel_buf_cap)

    (( apply_abs == 1 && want > cap_abs )) && want=$cap_abs   # 绝对上限(仅自动)

    # v2.9.2: 内存感知上限 — 单个窗口不超过可用内存的 40%, 防低内存机被大窗口拖垮
    cap_ram=$(available_ram_bytes)
    if (( cap_ram > 0 )); then
        local ram_lim=$(( cap_ram * 2 / 5 ))   # 40%
...
iperf3_error_reason() {
    ...
    if [[ -z "$reason" && -s "${out}.err" ]]; then
        reason=$(grep -iE 'error|cannot|unable|refused|denied|no route|timed out|timeout' "${out}.err" 2>/dev/null | head -1)
        [[ -z "$reason" ]] && reason=$(head -1 "${out}.err" 2>/dev/null)
    fi
    echo "$reason"
}
...
单流优化参数 (v2.4.0 新增):
  ...
  --window-max SIZE            自动窗口的绝对上限 (默认 32M; ≤2G 链路足够, 防窗口算过大)
