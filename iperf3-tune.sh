#!/usr/bin/env bash
#
# iperf3-tune.sh — iperf3 高速低重传优化工具 (v2.9.1)
#
...
WINDOW_MAX="64M"           # v2.8.0: 自动窗口的绝对上限 (1G 宽带足够; 防窗口算过大跑不起来)
...
#   上限 = min(WINDOW_MAX[仅自动计算时], 内核 wmem/rmem 上限)
...
clamp_window_bytes() {
    ...
    cap_abs=$(to_bytes "$WINDOW_MAX"); (( cap_abs > 0 )) || cap_abs=$((64*1048576))
...
单流优化参数 (v2.4.0 新增):
  ...
  --window-max SIZE            自动窗口的绝对上限 (默认 64M; 1G 宽带足够, 防窗口算过大)
