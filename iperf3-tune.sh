#!/usr/bin/env bash
#
# iperf3-tune.sh — iperf3 高速低重传优化工具 (v2.5.0)
#
...
sender_available_congestion() {
    if sender_is_remote; then
        ssh_remote "sysctl -n net.ipv4.tcp_available_congestion_control" 2>/dev/null || echo "bbr cubic"
    else
...
sender_current_congestion() {
    if sender_is_remote; then
        ssh_remote "sysctl -n net.ipv4.tcp_congestion_control" 2>/dev/null || echo "$CONGESTION"
    else
...
set_sender_congestion() {
    local cc="$1"
    if sender_is_remote; then
        ssh_remote "modprobe tcp_${cc} 2>/dev/null; sysctl -w net.ipv4.tcp_congestion_control=${cc}" >/dev/null 2>&1
    else
...
remote_cpu_candidates() {
    [[ -n "$SSH_HOST" ]] || { echo ""; return; }
    local rif="" rnode="" rlist="" rnproc=""
    rif=$(ssh_remote "ip -o -4 route show default 2>/dev/null" 2>/dev/null \
        | awk '{for(i=1;i<=NF;i++) if($i=="dev"){print $(i+1); exit}}')
    rnproc=$(ssh_remote "nproc" 2>/dev/null | tr -dc '0-9')
    if [[ -n "$rif" ]]; then
        rnode=$(ssh_remote "cat /sys/class/net/$rif/device/numa_node 2>/dev/null" 2>/dev/null | tr -dc '0-9-')
        if [[ "$rnode" =~ ^[0-9]+$ ]]; then
            rlist=$(ssh_remote "cat /sys/devices/system/node/node$rnode/cpulist 2>/dev/null" 2>/dev/null)
        fi
    fi
    if [[ -z "$rlist" ]]; then
...
report_zerocopy() {
    ...
    if [[ "$DIRECTION" == "reverse" ]]; then
        ...
        if [[ -n "$SSH_HOST" ]]; then
            local rv
            rv=$(ssh_remote "iperf3 --version 2>/dev/null | head -1" 2>/dev/null)
            if [[ -n "$rv" ]]; then
                log "  server iperf3: ${rv} (需 ≥3.x 才支持 zerocopy 协商)"
            else
                warn "  无法读取 server 端 iperf3 版本, 请自行确认其支持 zerocopy"
            fi
        else
...
apply_nic_tuning() {
    ...
    # v2.4.0: 停掉 irqbalance, 否则它会周期性打乱我们设的 IRQ 亲和
    if command -v systemctl >/dev/null 2>&1; then
        systemctl stop irqbalance >/dev/null 2>&1 && ok "  已停止 irqbalance (保持 IRQ 亲和稳定)" || true
    fi
...
典型示例:
  ...
  ${C_CYAN}# 单流 + jumbo frame (两端都需支持 9000 MTU)${C_RESET}
  sudo $SCRIPT_NAME optimize --single --mtu 9000 \\
