...
CONGESTION="bbr"
MSS=1448                       # 估算重传率时用的 MSS (TCP 模式无 packets 字段, 必须估算)
                               # 1448 = 1500 MTU - 20 IP - 20 TCP - 12 TS, 千兆典型值
                               # jumbo frame 改 8960 等
RETRANS_THRESHOLD=3.0          # %，超过即视为不健康
                               # 1.0 适合裸金属/同机房, 3.0 适合公网 (BBR 默认能容忍)
                               # 实际是否健康还要看 RTT, 见 bench 末尾分析
...
# ---------------------------------------------------------------------------
# RTT / 丢包探测 + 链路诊断分析
# (用于 bench 完成后给用户一个明确的"链路状况"结论, 而不只是数字)
# ---------------------------------------------------------------------------
# 测目标的 RTT 和丢包率 (用 ping). 返回 "avg_ms|loss_pct"
measure_rtt() {
    local target="$1"
    command -v ping >/dev/null 2>&1 || { echo "0|0"; return; }

    local pong
    # -c 10 个包, -i 0.2 间隔, -W 2 单包超时
    pong=$(ping -c 10 -i 0.2 -W 2 "$target" 2>&1) || true

    # avg 在 "min/avg/max/mdev = X/Y/Z/W" 这一行的第二个数字
    # 用 grep -oE 提取形如 "数字/数字/数字" 的三段, 取第 2 段
    local rtt
    rtt=$(echo "$pong" | grep -oE '[0-9]+\.[0-9]+/[0-9]+\.[0-9]+/[0-9]+\.[0-9]+' \
                        | head -1 | awk -F'/' '{print $2}')

    # loss% : 找 "X% packet loss" 中的 X
    local loss
...
# 链路诊断分析: 用人话告诉用户当前网络处于什么状态, 还能不能优化
# 参数: state_json (含 single+best), nic_speed_mbps, rtt_ms, loss_pct
analyze_link() {
    local state_json="$1"
    local nic_speed="${2:-0}"
    local rtt="${3:-0}"
    ...
    # 4. 重传率评级 (按链路类型分档, 因为 LAN 和跨境的"健康标准"完全不同)
    ...
    # case B: NIC 是 10G+ 但只跑出 ~1G → 中间链路 1G 限速
...
# 从 iperf3 JSON 提取测量结果
# 返回格式: bandwidth_mbps|retransmits|estimated_segments|bytes_sent
#
# 关键修复 (v2.1): iperf3 TCP 模式 *不输出 packets 字段* (只有 UDP 模式才有),
# v2.0 用 packets 算分母, 永远是 0, 导致重传率永远显示 0.000%。
# 这里改成 bytes/MSS 估算总段数, MSS 默认 1448 (1500 MTU - 20 IP - 20 TCP - 12 TS),
# 可通过 --mss 或 IPERF3_TUNE_MSS env 覆盖。
parse_iperf3() {
    local f="$1"
    local mss="${MSS:-1448}"
    ...
    jq -r --argjson mss "$mss" '
        ...
        # 重传数: 优先 sum_sent (iperf3 自己算的总和), fallback 到逐 stream 求和
        def retrans:
...
find_optimal_parallel() {
    ...
    for streams in 1 2 4 8 16 32; do
        ...
        # 早停 1: 重传率超阈值
        # 但是单流 (streams=1) 不允许早停 —— 多流的意义就是分散丢包,
        # 单流 5% 重传时 4 流可能 0.5%, 不试就不知道. 除非离谱 (>10%).
...
cmd_watch() {
    ...
    while true; do
        ...
        if [[ "$count" != "$last_count" ]]; then
            ...
            jq -r '.results[] | select(.ok) | "  \(.streams) 流: \(.bandwidth_mbps) Mbps   重传 \(.retransmits) 次 (\(.retrans_rate_pct)%)   评分 \(.score)"' \
            ...
            if [[ "$status" == "finished" ]]; then
                ...
                best_rt=$(jq -r '.best.retransmits // "?"' "$PROGRESS_FILE")
                best_rr=$(jq -r '.best.retrans_rate_pct // "?"' "$PROGRESS_FILE")
                printf '%s最优: %s 流, %s Mbps, 重传 %s 次 (%s%%)%s\n' \
                    "${C_GREEN}${C_BOLD}" "$best_s" "$best_bw" "$best_rt" "$best_rr" "${C_RESET}" >&2
                break
            fi
            last_count=$count
        fi
...
cmd_bench() {
    ...
    # 0) 先 ping 一下, 拿 RTT 和丢包率用于后面的分析
    log "0) 探测目标 RTT..."
    local rtt_loss; rtt_loss=$(measure_rtt "$SERVER")
    local rtt loss
    IFS='|' read -r rtt loss <<<"$rtt_loss"
    if num_gt "$rtt" 0; then
        ok "RTT ${rtt} ms, ping 丢包 ${loss}%"
    else
        warn "ping 不可用或被防火墙拦了, 跳过 RTT 测量"
    fi
    echo "" >&2
    ...
    printf '  单流:   %s%.2f Mbps%s  重传 %s 次 (%s%%)\n' \
        "${C_CYAN}" "$s_bw" "${C_RESET}" "$s_rt" "$s_rr" >&2
    printf '  多流:   %s%.2f Mbps%s  重传 %s 次 (%s%%)  (%s 流)\n' \
        "${C_GREEN}" "$best_bw" "${C_RESET}" "$best_rt" "$best_rr" "$best_s" >&2
    ...
    local out; out=$(jq -n \
        ...
        --arg improve "$improvement" --argjson mss "${MSS:-1448}" \
        --arg rtt "$rtt" --arg loss "$loss" \
        '{timestamp:$ts, mss:$mss, rtt_ms:($rtt|tonumber), loss_pct:($loss|tonumber),
          single:$single, best:$optimal.best, all:$optimal.all, improvement:$improve}')
    save_state "$out"
    [[ $JSON_OUT -eq 1 ]] && echo "$out"

    # 3) 链路诊断分析
    local iface nic_speed
    iface=$(detect_primary_iface 2>/dev/null || echo "")
    if [[ -n "$iface" && -r "/sys/class/net/$iface/speed" ]]; then
        nic_speed=$(cat "/sys/class/net/$iface/speed" 2>/dev/null)
        [[ "$nic_speed" =~ ^-?[0-9]+$ ]] || nic_speed=0
        [[ "$nic_speed" -lt 0 ]] && nic_speed=0
    fi
    nic_speed="${nic_speed:-0}"
    analyze_link "$out" "$nic_speed" "$rtt" "$loss"
}
...
${C_BOLD}选项${C_RESET}
  ...
  --mss N                   估算重传率用的 MSS, 默认 1448 (TCP 模式 iperf3
                            不输出包数, 必须用 bytes/MSS 估算)
                            jumbo frame (MTU 9000) 改 8948 或 8960
...
parse_args() {
    ...
    [[ "$REPEATS" =~ ^[0-9]+$ ]] && [[ $REPEATS -ge 1 ]] \
        || die "--repeats 至少 1"
    [[ "$MSS" =~ ^[0-9]+$ ]] && [[ $MSS -ge 64 && $MSS -le 65535 ]] \
        || die "--mss 范围 64..65535"
}
