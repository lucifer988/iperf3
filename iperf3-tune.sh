#!/usr/bin/env bash
#
# iperf3-tune.sh — iperf3 高速低重传优化工具 (v2.9.3)
#
...
clamp_window_bytes() {
    ...
    # v2.9.2/2.9.3: 内存感知上限 — 防低内存机被大窗口拖垮。
    #   单流模式(--single/--scan): 单个窗口 ≤ 可用内存×40%
    #   多流模式: 窗口会被 N 条流同时使用, 故 ≤ 可用内存×40% / min(MAX_PARALLEL,8)
    ...
    if (( cap_ram > 0 )); then
        ...
        local ram_lim=$(( cap_ram * 2 / 5 / div ))   # 40% / div
...
tuning_impact_items() {
    ...
    if [[ $NIC_TUNE -eq 1 ]]; then
        ...
        items+=("开启网卡 offload (gro/tso/gso/sg 等$([[ $ENABLE_LRO -eq 1 ]] && echo '，含 LRO'))，调大 ring buffer (rollback 会还原)")
...
probe_iperf3_connectivity() {
    ...
    local -a a=(-c "$SERVER" -p "$PORT" -t 3 -O 0 --connect-timeout 5000 -J)
    [[ "$DIRECTION" == "reverse" ]] && a+=(-R)
    log "连通性探测: iperf3 -c ${SERVER} -p ${PORT} -t 3${DIRECTION:+ ($DIRECTION)} ..."
    if iperf3 "${a[@]}" > "$tmp" 2>"${tmp}.err"; then
        ...
        if [[ "$got" =~ ^[0-9.e+]+$ ]] && num_gt "$got" 0; then
            ok "连通性正常 (探测吞吐 $(awk -v b="$got" 'BEGIN{printf "%.0f", b/1000000}') Mbps)"
            return 0
        fi
...
cmd_bench() {
    ...
    # v2.9.3: 正式测试/扫描前先做一次快速连通性探测 (3s)。连不上就立刻带真实原因失败,
    # 不再浪费 1~2 分钟扫描全 0 结果并打印误导性的 "最优"。
    probe_iperf3_connectivity || {
