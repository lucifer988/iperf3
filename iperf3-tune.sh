#!/usr/bin/env bash
#
# iperf3-tune.sh — iperf3 高速低重传优化工具 (v3.0.0)
#
# 目标: 最大化 iperf3 测得带宽 (尤其单流 -R 下行), 最小化重传率。
# 调优偏激进, 但默认对生产环境更克制、更可控、可预演、可回退。
#
# v3.0.0 改动: ★ 以 NIC 目标利用率为反馈的闭环寻优 ★
#   1. --scan* 先完成单流参数寻优, 再按 NIC 目标利用率决定是否自动进入多流寻优。
#   2. 单流达到目标(默认 NIC 的 85%)立即停止; 未达到则自动尝试 -P 1/2/4/8/16/32。
#   3. 最终 state.json 合并单流和多流样本, 保存阶段、目标、利用率和停止原因, 便于审计。
#   4. --single 仍严格只测单流; NIC 速率无法读取时不伪造利用率, 但 scan 仍会继续多流验证。
#
# v2.9.5 及更早版本的修复记录见 git/发行包历史。
#   1. ★★★ 修复 "/proc/sys/net 不可写" 误报: 不再用不可靠的目录 -w 判断, 改为无副作用试写
#          一个标量 sysctl; 并在应用后真正读回验证 rmem/wmem_max、拥塞算法、qdisc 是否生效。
#   2. ★★★ 修复 "多流没有意义" 的误导结论: 单流寻优(--scan/--single)根本没测 -P 2/4/8,
#          不再伪造 "多流提升 0.0% (1流→1流)", 改为明确提示"未测多流"并给出手动测试命令。
#   3. ★ 安装文档改用 bash install.sh (不依赖可执行位/shebang, 规避 "command not found")。
#   注: tuning_impact_items 崩栈已在 2.9.4 修复; 拥塞算法切换会读回校验(2.9.3)。
#
# v2.9.4 改动 (vs 2.9.3): ★ 实测日志反馈修复 ★
#   1. ★★★ 修复 tuning_impact_items 崩栈: items+=("...$([[ ENABLE_LRO ]] && echo)...") 里
#          内嵌命令替换在 ENABLE_LRO=0 时退出码 1, set -e 下让赋值失败触发 ERR trap。改为预算变量。
#   2. ★★★ 窗口改按"实测吞吐"定容: 连通性探测顺带测出真实链路速率, 用户没给 --bandwidth 时
#          据此估窗口(实测×2, 不超 NIC 标称), 不再用网卡 10G 标称把窗口算成 96M。
#          实测: 该改动把 0.5GB 内存机上 30s 测试从塌到个位数 → 稳定贴合真实链路。
#   3. ★★★ 内存感知上限按可用内存分级收紧: <1GB 用 12%、1~4GB 用 20%、≥4GB 用 30%,
#          杜绝大窗口在低内存机持续传输时把吞吐拖崩。
#   4. ★★ 窗口候选倍数加入 0.5×BDP, 受限链路上能扫到更小更稳的窗口 (不再只剩一个候选)。
#
# v2.9.3 改动 (vs 2.9.2): ★ 修复实测崩溃 + SSH 密码认证 + 连通性快速失败 ★
#   1. ★★★ 修复 measure_rtt 等在 set -Eeuo pipefail 下崩溃: 管道里 grep 无匹配返回 1,
#          pipefail 把裸赋值置失败触发 errexit 让整脚本崩栈 (用户实测的根因)。一律 || true。
#   2. ★★★ SSH 密码模式强制走密码认证 (PreferredAuthentications=password、关 pubkey/agent),
#          否则 ssh 先试 publickey 直接报 "Permission denied (publickey)", 密码根本没用上。
#   3. ★★★ 正式测试/扫描前先做 3s 连通性探测: 连不上就立刻带 iperf3 真实原因失败,
#          不再浪费 1~2 分钟扫描全 0 并打印误导性 "最优"。
#   4. ★★ 窗口绝对上限回调到 96M (够单流高 RTT 打满 ~2G), 内存感知上限改为按"是否单流"
#          区分: 单流用满 40% 内存预算, 多流再除以并发数(≤8) 防低内存机 OOM。
#   5. ★ 针对"2G 拉满单线程"目标: 强烈建议显式 --bandwidth 2000, 否则会用 NIC 标称(可能 10G)
#          估窗口再被上限夹小; 给了 --bandwidth 才能按真实 BDP 精确定窗。
#
# v2.9.2 改动 (vs 2.9.1): ★ 低内存/高 RTT 实战修复 + 可诊断性 ★
#   1. ★★★ iperf3 失败原因不再被吞: run_iperf3 保留 stderr, 失败时打印 iperf3 真实报错
#          (-J 的 .error 或 stderr), 不再只是 "第 N 次测试失败" 让人无从排查。
#   2. ★★★ 窗口绝对上限降到 32M (≤2G 链路足够), 并新增"内存感知"上限: 单窗口不超过
#          可用内存的 40%, 低内存机 (如 0.5GB 可用) 不再被大窗口拖垮。
#   3. ★★ install.sh 安装后打印实际版本 + 跑 selftest, 并在 PATH 指向旧二进制时告警,
#          避免再出现"装了新版却跑到 /usr/local/sbin 旧版"的情况。
#
# v2.9.1 改动 (vs 2.9.0): ★ 全失败处理再加固 (消除任何歧义) ★
#   1. find_optimal_parallel 无成功样本时由 return 0 改为 return 1, 即使被单独调用,
#      函数本身也以非零返回, 不可能再让调用方误判成功。
#   2. cmd_bench 显式捕获其返回码 (opt_rc), 与 best!=null 双重判定; 任一表明失败即
#      写 status:"failed" 并 die (非零退出)。
#   3. state.best 在多流全失败但单流成功时回退到单流结果, null 不再向 status/分析下游传播。
#
# v2.9.0 改动 (vs 2.8.0): ★ 生产安全与正确性加固 (基于 code review) ★
#   1. ★★★ 全失败不再误判成功: 单流+多流测试若无任何成功样本, 写入 status:"failed"
#          的状态 JSON 并以非零退出, 不再打印 "最优: null 流" 误导自动化系统。
#   2. ★★★ 拥塞算法切换失败真正算失败: set_sender_congestion 现在读回校验是否生效,
#          失败 return 1; 扫描会正确跳过未生效的算法; state 记录 effective_congestion。
#   3. ★★★ 网卡改动完整可回退: 备份并在 rollback 中还原 offload(含 LRO)/ring buffer/
#          RPS/XPS/IRQ 亲和; LRO 默认不再开启 (转发/桥接/虚拟化不友好), 需 --enable-lro。
#   4. ★★★ 新增 --no-nic-tune: 只调 sysctl, 完全不动网卡硬件 (offload/ring/RPS/IRQ)。
#   5. ★★★ CPU mask 不再溢出: 用逗号分组的 32bit cpumask 生成器, 64C/128C/192C 机器
#          的 RPS/XPS/IRQ 亲和不再因 bash 位移回绕而配错。
#   6. ★★ 远端脚本路径改用远端 mktemp 私有文件 (0600), 路径记录在本地 state, 不再用固定
#          /tmp/iperf3-tune.sh, 避免多用户机器上的覆盖/竞争/符号链接风险。
#   7. ★ iperf3-tuned 守护安装前检测 systemd, 非 systemd 系统给出明确报错而非中途失败。
#
# v2.8.0 改动 (vs 2.7.2): ★ 修复 TCP 窗口算过大导致 iperf3 跑不起来 ★
#   1. ★★★ 窗口不再无上限: 自动窗口从 2.5×BDP 改为 2.0×BDP, 并新增绝对上限
#          WINDOW_MAX (默认 64M, 可 --window-max 改)。1G 宽带 BDP 很小, 64M 远够,
#          根因是网卡标称 10G/25G 被当成实际带宽算出几百 MB 窗口。
#   2. ★★★ 窗口与内核缓冲上限联动: 设 -w 前夹到 min(算出值, net.core.wmem_max,
#          net.core.rmem_max), 保证请求值不超内核上限, 杜绝"窗口设置失败/被静默裁剪"。
#   3. ★★★ 系统缓冲仍是默认小值时(< 2MiB)不再硬塞 -w, 而是不带 -w 交给内核自动扩张
#          (autotune 比钉死一个小窗口吞吐更好); 提示先 tune 调大缓冲。
#   4. ★★ 手动 --window 现在校验+夹取: 格式不对则忽略并回退自动; 超内核上限自动夹小
#          (并告警), 不再直接把 iperf3 跑挂。
#   5. ★★ --scan-window 候选同样按上限夹取去重, 不会再生成跑不起来的超大候选。
#   6. ★ 新增 --window-max SIZE 覆盖窗口绝对上限 (默认 64M)。
#
# 用法:
...
WINDOW_MAX="96M"           # v2.9.3: 绝对上限 96M — 够单流在高 RTT 下打满 ~2G (2Gbit×200ms≈48MB BDP×2)
                           #         实际还会被"内核 wmem/rmem 上限"和"内存感知上限"进一步夹小
WINDOW_MULT="2.0"          # v2.8.0: 自动窗口倍数 = WINDOW_MULT × BDP (原 2.5, 降到 2.0 更稳)
readonly WINDOW_AUTOTUNE_MIN=2097152  # v2.8.0: 内核缓冲低于此(2MiB)时不固定 -w, 交给内核自动调更稳
BANDWIDTH=0                # 链路容量提示 Mbit/s, 用于自动算窗口; 0=用 NIC 标称速率
PROBE_MBPS=0               # v2.9.4: 连通性探测实测吞吐 (Mbit/s); 用户没给 --bandwidth 时据此定窗口
...
SCAN_REQUESTED=0           # v3.0.0: scan 请求与 SINGLE_ONLY 解耦
...
TARGET_UTILIZATION=85       # NIC 利用率闭环目标百分比 (scan 模式)
...
# v2.9.0: 网卡硬件调优开关
NIC_TUNE=1                 # 是否调网卡硬件 (offload/ring/RPS/XPS/IRQ); --no-nic-tune 只调 sysctl
ENABLE_LRO=0              # 是否开启 LRO (默认关: 对转发/桥接/虚拟化/抓包不友好); --enable-lro 开
SSH_ASK_PASS=0             # v2.7.0: 交互式读取 SSH 密码 (不进 history/argv)
SSH_REACHABLE=-1           # SSH 可达性缓存: -1 未检测 / 0 不可达 / 1 可达 (内部用)
REMOTE_SUDO_MODE="unknown" # 远端提权模式: root | sudo-nopass | sudo-pass | none | unknown
REMOTE_IPERF3_STARTED=0      # 本次流程是否临时拉起了远端 iperf3 -s
_SSH_PASS_TMP=""           # 内部: --ssh-ask-pass 生成的临时密码文件, 退出时清理
PRIMARY_IFACE=""
# v3.0.0: adaptive scan 的反馈控制变量; 仅在闭环调度期间启用
ADAPTIVE_TARGET_BW=0
...
# v2.9.0: 检测系统是否用 systemd (守护进程的 timer 安装/卸载需要它)
have_systemd() {
    command -v systemctl >/dev/null 2>&1 && [[ -d /run/systemd/system ]]
}

# v2.9.5: 可靠判断 sysctl 是否真的可写 (直接判断 /proc/sys/net 目录 -w 不可靠, 会误报)。
# 做法: 把一个标量 sysctl 原样写回 (无副作用), 成功即可写。
sysctl_writable() {
    command -v sysctl >/dev/null 2>&1 || return 1
    local cur
    cur=$(sysctl -n net.core.somaxconn 2>/dev/null) || return 1
    [[ "$cur" =~ ^[0-9]+$ ]] || return 1
    sysctl -w "net.core.somaxconn=${cur}" >/dev/null 2>&1
}

# v2.7.0: 退出时清理临时密码文件 (--ssh-ask-pass 及 --detach 转存的密码文件)
_cleanup_tmp() {
    [[ -n "${_SSH_PASS_TMP:-}" && -f "${_SSH_PASS_TMP}" ]] && rm -f "${_SSH_PASS_TMP}" 2>/dev/null || true
    [[ -n "${_SUDO_PASS_TMP:-}" && -f "${_SUDO_PASS_TMP}" ]] && rm -f "${_SUDO_PASS_TMP}" 2>/dev/null || true
    [[ -n "${IPERF3_TUNE_DETACH_PWFILE:-}" && -f "${IPERF3_TUNE_DETACH_PWFILE}" ]] && rm -f "${IPERF3_TUNE_DETACH_PWFILE}" 2>/dev/null || true
    [[ -n "${IPERF3_TUNE_DETACH_SUDO_PWFILE:-}" && -f "${IPERF3_TUNE_DETACH_SUDO_PWFILE}" ]] && rm -f "${IPERF3_TUNE_DETACH_SUDO_PWFILE}" 2>/dev/null || true
    return 0
}
...
preflight_check() {
    ...
    if cmd_mutates_system "$cmd"; then
        ...
        # 4) 容器/受限环境: sysctl 真的写不动时提前提示 (用无副作用试写, 不再用不可靠的目录 -w 判断)
        if [[ "$cmd" == "tune" || "$cmd" == "optimize" ]]; then
            if ! sysctl_writable; then
                warn "sysctl 写入测试失败 (容器或受限内核?): sysctl 调优可能无效"
...
# ---------------------------------------------------------------------------
# v2.9.0: Linux cpumask 生成 (逗号分组的 32bit 十六进制, 高位组在前)
#   bash 的 $((1<<n)) 在 64 核及以上会溢出/回绕, 内核 rps_cpus/xps_cpus/smp_affinity
#   也要求 >32 CPU 用逗号分组写法 (如 "ffffffff,ffffffff" 表示 64 位全置位)。
# ---------------------------------------------------------------------------
# 置位 [0, n) 的所有 CPU 位
cpu_mask_all() {
    awk -v n="$1" 'BEGIN{
        ...
        for(g=groups-1; g>=0; g--){
            lo=g*32; cnt=n-lo; if(cnt>32)cnt=32;
            if(cnt<=0)val=0; else if(cnt>=32)val=4294967295; else val=(2^cnt)-1;
            out = out (g<groups-1 ? "," : "") sprintf("%x", val);
        }
        print out;
    }'
}
# 只置位单个 CPU c (总核数 n 决定分组宽度)
cpu_mask_single() {
    awk -v c="$1" -v n="$2" 'function p2(e,  r,i){r=1;for(i=0;i<e;i++)r*=2;return r}
    BEGIN{
        if(n+0<=0)n=c+1;
        groups=int((n+31)/32); if(groups<1)groups=1;
        tgt=int(c/32); bit=c%32; out="";
        for(g=groups-1; g>=0; g--){
            val=(g==tgt)? p2(bit) : 0;
            out = out (g<groups-1 ? "," : "") sprintf("%x", val);
        }
...
# v2.9.0: 我们会改动的网卡 offload 特性: 短名(ethtool -K 用) -> 长名(ethtool -k 显示用)
# 备份/还原据此精确定位, 避免把 ethtool -k 的长名误传给 ethtool -K。
readonly NIC_OFFLOAD_SHORT="gro tso gso sg rx tx rxhash rxvlan txvlan lro"
_nic_offload_long() {
    case "$1" in
...
# 读某特性当前 on/off (读不到回显空)
_nic_feat_state() {
    local iface="$1" long="$2"
    ethtool -k "$iface" 2>/dev/null | awk -F': ' -v f="$long" '
        $1==f { v=$2; sub(/ \[fixed\]/,"",v); gsub(/ /,"",v); print v; exit }'
}
...
check_dependencies() {
    ...
    # sshpass 是按需依赖, 仅 --ssh-pass / --ssh-pass-file / --ssh-ask-pass / --sudo-pass / --sudo-pass-file / --sudo-ask-pass 时需要
    if [[ -n "$SSH_PASS" || -n "$SSH_PASS_FILE" || -n "$SUDO_PASS" || -n "$SUDO_PASS_FILE" || $SUDO_ASK_PASS -eq 1 ]] && ! command -v sshpass >/dev/null 2>&1; then
        if [[ $AUTO_INSTALL -eq 1 && $EUID -eq 0 ]]; then
            log "需要 sshpass, 自动安装..."
...
# v2.9.4: 主网卡标称速率 (Mbit/s); 读不到或异常(虚拟网卡常报 -1)回 0
detect_nic_speed_mbps() {
    local iface s
    iface=$(detect_primary_iface) || iface=""
    [[ -z "$iface" ]] && { echo 0; return 0; }
    s=$(cat "/sys/class/net/$iface/speed" 2>/dev/null || echo 0)
    [[ "$s" =~ ^[0-9]+$ ]] && (( s > 0 )) && echo "$s" || echo 0
}
...
# v2.8.0: 把人类可读大小 (96M / 64K / 1G / 1048576) 解析成字节整数; 非法输入返回 0
to_bytes() {
    awk -v s="$1" 'BEGIN{
        s=toupper(s); gsub(/[ \t]/,"",s);
        if(s ~ /^[0-9]+(\.[0-9]+)?[KMG]?$/){
            u=""; if(s ~ /[KMG]$/){u=substr(s,length(s),1); s=substr(s,1,length(s)-1)}
            v=s+0;
...
# v2.8.0: 字节 → 形如 "16M" 的整数 MiB (至少 1M)
to_human_mib() {
    awk -v b="$1" 'BEGIN{ m=int(b/1048576 + 0.5); if(m<1)m=1; printf "%dM", m }'
}

# v2.8.0: 当前内核 SO_*BUF 上限 (字节) = min(wmem_max, rmem_max); 读不到任一返回另一个; 都读不到返回 0
kernel_buf_cap() {
    local w r
    ...
    [[ "$w" =~ ^[0-9]+$ ]] || w=0
    [[ "$r" =~ ^[0-9]+$ ]] || r=0
    if   (( w > 0 && r > 0 )); then echo $(( w < r ? w : r ))
    elif (( w > 0 ));         then echo "$w"
    elif (( r > 0 ));         then echo "$r"
    else echo 0; fi
}

# v2.9.2: 可用内存字节数 (MemAvailable); 读不到回 0
available_ram_bytes() {
    local kb
    kb=$(awk '/^MemAvailable:/{print $2; exit}' /proc/meminfo 2>/dev/null)
    [[ "$kb" =~ ^[0-9]+$ ]] && echo $(( kb * 1024 )) || echo 0
}

# v2.8.0: 把期望窗口字节数夹到安全区间, 杜绝 iperf3 -w 设过大跑不起来。
#   上限 = min(WINDOW_MAX[仅自动计算时], 内核 wmem/rmem 上限, 可用内存×0.4)
#   若内核缓冲还是默认小值 (< WINDOW_AUTOTUNE_MIN) 则返回 0
#     → 调用方据此"不加 -w", 交给内核自动扩张 (比钉死一个小窗口吞吐更好)
# 入参: $1=期望字节数  $2=是否套用绝对上限 WINDOW_MAX (1=是[默认, 自动计算用], 0=否[手动 --window 用])
# 出参: 安全字节数 (0 = 不要设 -w)
clamp_window_bytes() {
    local want="$1" apply_abs="${2:-1}"
    [[ "$want" =~ ^[0-9]+$ ]] || { echo 0; return 0; }
    local cap_abs cap_kern cap_ram
    cap_abs=$(to_bytes "$WINDOW_MAX"); (( cap_abs > 0 )) || cap_abs=$((96*1048576))
    cap_kern=$(kernel_buf_cap)

    (( apply_abs == 1 && want > cap_abs )) && want=$cap_abs   # 绝对上限(仅自动)

    # v2.9.2/2.9.4: 内存感知上限 — 防低内存机被大窗口拖垮 (实测: 0.5GB 机上 96M 窗口在
    # 30s 持续传输时把吞吐从 700Mbps 拖崩到个位数)。可用内存越小, 占比越保守。
    #   单流: 可用内存 ≥4GB 用 30%; 1~4GB 用 20%; <1GB 用 12%
    #   多流: 再除以 min(MAX_PARALLEL,8)
    cap_ram=$(available_ram_bytes)
    if (( cap_ram > 0 )); then
        local frac_num=3 frac_den=10                 # 默认 30%
        if   (( cap_ram < 1073741824 )); then frac_num=12; frac_den=100   # <1GB → 12%
        elif (( cap_ram < 4294967296 )); then frac_num=20; frac_den=100   # 1~4GB → 20%
        fi
        local div=1
        if [[ ${SINGLE_ONLY:-0} -ne 1 && ${SCAN_WINDOW:-0} -ne 1 && ${SCAN_CONGESTION:-0} -ne 1 \
              && ${SCAN_CPU:-0} -ne 1 && ${SCAN_ALL:-0} -ne 1 ]]; then
            div=${MAX_PARALLEL:-1}; (( div > 8 )) && div=8; (( div < 1 )) && div=1
        fi
        local ram_lim=$(( cap_ram * frac_num / frac_den / div ))
        (( ram_lim >= 262144 && want > ram_lim )) && want=$ram_lim
    fi

    if (( cap_kern > 0 )); then
        if (( cap_kern < WINDOW_AUTOTUNE_MIN )); then
            echo 0; return 0                         # 缓冲太小: 不固定 -w, 交给 autotune
        fi
        (( want > cap_kern )) && want=$cap_kern      # 不超内核上限 → 内核不会裁剪
    fi

    (( want < 262144 )) && want=262144               # 下限 256K 保护
    echo "$want"
}

# 由带宽(Mbit/s)和 RTT(ms) 算 WINDOW_MULT×BDP 的窗口并夹到安全区间, 输出形如 "16M";
# 若当前内核缓冲过小不宜固定窗口, 输出空字符串 (调用方据此不加 -w)
calc_window_human() {
    local mbit="$1" rtt_ms="$2" want safe
    want=$(awk -v b="$mbit" -v r="$rtt_ms" -v m="$WINDOW_MULT" 'BEGIN{
        printf "%d", (b*1000000/8)*(r/1000)*m
    }')
    safe=$(clamp_window_bytes "$want")
    if [[ "$safe" == "0" ]]; then echo ""; else to_human_mib "$safe"; fi
}
...
resolve_pin_and_window() {
    ...
    local _pin_tail="本机核 ${C_BOLD}${PIN_CPU}${C_RESET}${REMOTE_PIN_CPU:+  远端核 ${C_BOLD}${REMOTE_PIN_CPU}${C_RESET}}; 零拷贝=$([[ $ZEROCOPY -eq 1 ]] && echo on || echo off)"

    # v2.8.0: 手动窗口先校验格式, 再按内核缓冲上限夹取, 杜绝设过大跑不起来
    if [[ -n "$WINDOW" ]]; then
        local _wb _sb
        _wb=$(to_bytes "$WINDOW")
        if (( _wb <= 0 )); then
            warn "手动 TCP 窗口 '${WINDOW}' 格式无法识别 (示例 16M / 64M / 256K), 忽略并改用自动计算"
            WINDOW=""
        else
            _sb=$(clamp_window_bytes "$_wb" 0)
            if [[ "$_sb" == "0" ]]; then
                IPERF3_WINDOW=""
                warn "手动窗口 ${C_BOLD}${WINDOW}${C_RESET} 超过当前内核缓冲上限, 且系统缓冲仍是默认小值; 已改用内核自动窗口 (建议先 ${C_CYAN}${SCRIPT_NAME} tune${C_RESET} 调大 wmem_max/rmem_max); ${_pin_tail}"
                return 0
            fi
            IPERF3_WINDOW=$(to_human_mib "$_sb")
            if (( _sb < _wb )); then
                warn "手动窗口 ${WINDOW} 超过当前内核缓冲上限, 已自动夹到 ${C_BOLD}${IPERF3_WINDOW}${C_RESET} (避免 iperf3 -w 设置失败); ${_pin_tail}"
            else
                log "TCP 窗口: 手动 ${C_BOLD}${IPERF3_WINDOW}${C_RESET}; ${_pin_tail}"
            fi
            return 0
        fi
    fi
    ...
    if num_gt "$rtt" 0 && num_gt "$bw" 0; then
        ...
        if [[ -n "$IPERF3_WINDOW" ]]; then
            log "自动 TCP 窗口: RTT=${rtt}ms × ${bw}Mbit → ${C_BOLD}${IPERF3_WINDOW}${C_RESET} (${WINDOW_MULT}×BDP, 上限 ${WINDOW_MAX}); ${_pin_tail}"
        else
            warn "当前系统缓冲仍是默认小值, 不固定 -w 改用内核自动扩张 (更稳, 想要更大窗口请先 ${C_CYAN}${SCRIPT_NAME} tune${C_RESET}); ${_pin_tail}"
        fi
    else
        IPERF3_WINDOW=""
...
# 在发送端设置拥塞算法 (必要时先 modprobe), 读回校验是否真正生效。
# v2.9.0: 未生效则 return 1 (扫描据此跳过, 避免"看似切换实则没切"导致结果失真)。
set_sender_congestion() {
    ...
    if sender_is_remote; then
        if remote_run "modprobe tcp_${cc} 2>/dev/null; sysctl -w net.ipv4.tcp_congestion_control=${cc} >/dev/null 2>&1; [ \"\$(sysctl -n net.ipv4.tcp_congestion_control 2>/dev/null)\" = \"${cc}\" ]"; then
            return 0
        fi
        warn "远端设置拥塞算法 ${cc} 失败或未生效 (SSH 不可达 / 权限不足 / 模块不可用)"
        return 1
    else
        modprobe "tcp_${cc}" >/dev/null 2>&1 || true
        sysctl -w "net.ipv4.tcp_congestion_control=${cc}" >/dev/null 2>&1 || true
        if [[ "$(sysctl -n net.ipv4.tcp_congestion_control 2>/dev/null)" == "$cc" ]]; then
            return 0
        fi
        warn "本机设置拥塞算法 ${cc} 失败或未生效 (模块未加载 / 内核不支持)"
        return 1
    fi
}
...
# 由 BDP 生成窗口候选 (单位 M), 以 1.0/1.5/2.0/2.5/3.0×BDP 为锚, 去重。
# v2.8.0: 每个候选都按 min(WINDOW_MAX, 内核缓冲上限) 夹取, 不再生成跑不起来的超大候选。
window_candidates() {
    local cap_abs cap_kern
    cap_abs=$(to_bytes "$WINDOW_MAX"); (( cap_abs > 0 )) || cap_abs=$((96*1048576))
    cap_kern=$(kernel_buf_cap)

    # 自定义列表: 同样校验+夹取+去重
    if [[ -n "$WINDOW_LIST" ]]; then
        local _out=() _w _wb _sb _h _seen=" "
        for _w in ${WINDOW_LIST//,/ }; do
            _wb=$(to_bytes "$_w"); (( _wb <= 0 )) && continue
            _sb=$(clamp_window_bytes "$_wb" 0); [[ "$_sb" == "0" ]] && continue
            _h=$(to_human_mib "$_sb")
            case "$_seen" in *" $_h "*) ;; *) _out+=("$_h"); _seen="$_seen$_h ";; esac
        done
        (( ${#_out[@]} > 0 )) && { echo "${_out[*]}"; return; }
        # 列表被全部夹没(系统缓冲过小)时, 落到下面的默认逻辑
    fi
    ...
    # 无法测速时给一组固定候选, 同样按上限夹取
    if ! num_gt "$rtt" 0 || ! num_gt "$bw" 0; then
        awk -v ca="$cap_abs" -v ck="$cap_kern" 'BEGIN{
            split("32 64 96 128 192 256", g, " ");
            for(i=1;i<=6;i++){
                want=g[i]*1048576;
                if(want>ca)want=ca; if(ck>0 && want>ck)want=ck;
                if(want<262144)want=262144;
                m=int(want/1048576+0.5); if(m<1)m=1;
    ...
    awk -v b="$bw" -v r="$rtt" -v ca="$cap_abs" -v ck="$cap_kern" 'BEGIN{
        bdp=(b*1000000/8)*(r/1000);
        split("0.5 1.0 1.5 2.0 3.0", f, " ");
        for(i=1;i<=5;i++){
            want=bdp*f[i];
            if(want>ca)want=ca; if(ck>0 && want>ck)want=ck;
            if(want<262144)want=262144;
...
single_stream_optimize() {
    ...
    # v2.9.0: 应用选定拥塞算法并记录"实际生效"值 (可能因模块缺失等没切成功)
    local cc_effective
    if set_sender_congestion "$best_cc"; then
        cc_effective="$best_cc"
    else
        cc_effective=$(sender_current_congestion)
        warn "无法切到最优拥塞算法 ${best_cc}, 实际生效: ${C_BOLD}${cc_effective}${C_RESET}"
    fi
    ...
    # v2.9.0: 正式确认前再次确保拥塞算法生效, 刷新 effective 值
    if set_sender_congestion "$best_cc"; then cc_effective="$best_cc"; else cc_effective=$(sender_current_congestion); fi
    log "  cc=${C_BOLD}${best_cc}${C_RESET}(实际生效 ${cc_effective})  window=${C_BOLD}${best_win:-auto}${C_RESET}  本机核=${C_BOLD}${best_cpu}${C_RESET}${REMOTE_PIN_CPU:+  远端核=${C_BOLD}${REMOTE_PIN_CPU}${C_RESET}}  zerocopy=$([[ $ZEROCOPY -eq 1 ]] && echo on || echo off)"
    CONGESTION="$cc_effective"
    ...
    jq -n \
        ...
        --arg cc "$best_cc" --arg cceff "$cc_effective" --arg win "${best_win:-auto}" \
        ...
        '{
            ...
            sender: $sender,
            best_congestion: $cc, effective_congestion: $cceff, best_window: $win,
...
# v3.0.0: 目标利用率反馈调度器。
# 先用单流扫描榨干参数; 未达到 NIC 目标时自动进入多流寻优。
adaptive_scan_optimize() {
    local iface nic_speed=0 target_bw=0 single multi multi_rc state
    iface=$(detect_primary_iface) || iface=""
    if [[ -n "$iface" && -r "/sys/class/net/$iface/speed" ]]; then
        nic_speed=$(cat "/sys/class/net/$iface/speed" 2>/dev/null || echo 0)
        [[ "$nic_speed" =~ ^-?[0-9]+$ ]] || nic_speed=0
        if (( nic_speed < 0 )); then nic_speed=0; fi
    fi
    if (( nic_speed > 0 )); then
        target_bw=$(awk -v n="$nic_speed" -v u="$TARGET_UTILIZATION" 'BEGIN {printf "%.3f", n*u/100}')
        ADAPTIVE_TARGET_BW="$target_bw"
        log "闭环目标: NIC ${nic_speed} Mbps × ${TARGET_UTILIZATION}% = ${target_bw} Mbps"
    else
        ADAPTIVE_TARGET_BW=0
        warn "无法读取 NIC 标称速率, 不伪造利用率目标; 将继续多流寻优并以评分选择最佳"
    fi

    single=$(single_stream_optimize)
    if [[ "$(jq -r '.best.ok // false' <<<"$single" 2>/dev/null)" != "true" ]]; then
        echo "$single"
        return 1
    fi
    local single_bw; single_bw=$(jq -r '.best.bandwidth_mbps // 0' <<<"$single")
    if [[ $SCAN_REQUESTED -eq 0 && $SINGLE_ONLY -eq 1 ]]; then
        ADAPTIVE_STOP_REASON="single-only"
        state=$(jq --argjson nic "$nic_speed" --argjson target "$target_bw" \
                   --arg stop "$ADAPTIVE_STOP_REASON" \
                   '. + {optimization: {strategy:"single", nic_speed_mbps:$nic, target_mbps:$target, stop_reason:$stop}}' <<<"$single")
        echo "$state"
        return 0
    fi
    if (( target_bw > 0 )) && num_ge "$single_bw" "$target_bw"; then
        ADAPTIVE_STOP_REASON="single-stream-target-reached"
        log "单流已达到 NIC 目标 (${single_bw} >= ${target_bw} Mbps), 停止继续加流"
        state=$(jq --argjson nic "$nic_speed" --argjson target "$target_bw" \
                   --arg stop "$ADAPTIVE_STOP_REASON" \
                   '. + {optimization: {strategy:"adaptive", nic_speed_mbps:$nic, target_mbps:$target, stop_reason:$stop}}' <<<"$single")
        echo "$state"
        return 0
    fi

    # 单流扫描在命令替换子 shell 中执行, 显式恢复选出的测试参数。
    CONGESTION=$(jq -r '.effective_congestion // .best_congestion // "cubic"' <<<"$single")
    IPERF3_WINDOW=$(jq -r '.best_window // ""' <<<"$single")
    PIN_CPU=$(jq -r '.best_cpu_local // ""' <<<"$single")
    REMOTE_PIN_CPU=$(jq -r '.best_cpu_remote // ""' <<<"$single")
    [[ "$IPERF3_WINDOW" == "auto" ]] && IPERF3_WINDOW=""
    SINGLE_ONLY=0
    log "单流 ${single_bw} Mbps 未达到目标, 自动进入多流寻优"
    multi_rc=0
    multi=$(find_optimal_parallel "$SERVER" "$PORT" "$DURATION" "$REPEATS" "$MAX_PARALLEL" "$target_bw" 2) || multi_rc=$?
    [[ -n "$multi" ]] || multi='{"all":[],"best":null}'
    if [[ "$multi_rc" -ne 0 && "$(jq -r '.best != null' <<<"$multi" 2>/dev/null)" != "true" ]]; then
        ADAPTIVE_STOP_REASON="multi-stream-no-success"
        state=$(jq --argjson nic "$nic_speed" --argjson target "$target_bw" \
                   --arg stop "$ADAPTIVE_STOP_REASON" --argjson m "$multi" \
                   '. + {all: ((.all // []) + ($m.all // [])), optimization: {strategy:"adaptive", nic_speed_mbps:$nic, target_mbps:$target, stop_reason:$stop}}' <<<"$single")
        echo "$state"
        return "$multi_rc"
    fi
    local best
    best=$(jq -n --argjson s "$single" --argjson m "$multi" \
              '[$s.best,$m.best] | map(select(.ok == true)) | sort_by(-.score) | .[0]')
    local multi_bw; multi_bw=$(jq -r '.best.bandwidth_mbps // 0' <<<"$multi")
    if (( target_bw > 0 )) && num_ge "$multi_bw" "$target_bw"; then
        ADAPTIVE_STOP_REASON="multi-stream-target-reached"
    else
        ADAPTIVE_STOP_REASON="parallel-search-exhausted"
    fi
    state=$(jq --argjson nic "$nic_speed" --argjson target "$target_bw" \
               --arg stop "$ADAPTIVE_STOP_REASON" --argjson m "$multi" --argjson b "$best" \
               '. + {all: ((.all // []) + ($m.all // [])), best:$b, optimization: {strategy:"adaptive", nic_speed_mbps:$nic, target_mbps:$target, stop_reason:$stop}}' <<<"$single")
    echo "$state"
    return 0
}
...
detect_cpu() {
    ...
    local irq_queues
    irq_queues=$(grep -E "${iface}(-|\$)" /proc/interrupts 2>/dev/null | wc -l) || true
...
measure_rtt() {
    local target="$1"
    command -v ping >/dev/null 2>&1 || { echo "0|0"; return 0; }
    local pong
    pong=$(ping -c 10 -i 0.2 -W 2 "$target" 2>&1) || true

    # v2.9.3: 这些管道含 grep, 不通时 grep 无匹配返回 1; set -o pipefail 下裸赋值会触发
    # errexit 让整个脚本崩 (用户实测的 measure_rtt 崩栈根因)。一律 || true 兜底。
    local rtt loss
...
analyze_link() {
    ...
    local s_bw best_s best_bw best_rr multi_tested
    ...
    # v2.9.5: 是否真的测过多流 (.all 里出现过 >1 的并发数); 单流寻优(--scan)只测 1 流
    multi_tested=$(jq -r '([.all[]?.streams] | map(select(.>1)) | length) > 0' <<<"$state_json" 2>/dev/null) || multi_tested=false
    ...
    if [[ "$multi_tested" != "true" ]]; then
        if [[ "$(jq -r '.optimization.strategy // ""' <<<"$state_json")" == "adaptive" ]]; then
            log "  本次闭环在单流阶段已达到目标, 未继续增加并发"
        else
            printf '  多流提升:     %s未测%s (本次为单流模式 --single, 未跑 -P 2/4/8)\n' \
                "${C_YELLOW}" "${C_RESET}" >&2
        fi
    fi
    ...
    if [[ "$multi_tested" != "true" && "$(jq -r '.optimization.strategy // ""' <<<"$state_json")" != "adaptive" ]]; then
        log "  本次为单流模式, ${C_BOLD}未测多流${C_RESET}; 想验证多流是否更快, 可单独跑:"
        log "    ${C_CYAN}for p in 1 2 4 8; do iperf3 -c ${SERVER:-<IP>} -p ${PORT:-5201} -R -t 30 -O 5 -P \$p; done${C_RESET}"
    elif [[ "$multi_tested" == "true" && "$best_s" == "1" ]]; then
        log "  寻优最优配置是 ${C_BOLD}单流${C_RESET} (已对比过多流), 说明单 TCP 连接已基本撑满链路"
        if [[ "$(jq -r '.optimization.stop_reason // ""' <<<"$state_json")" == "single-stream-target-reached" ]]; then
            log "  闭环判断: 单流已达到设定目标, 未继续增加并发"
        else
            log "  闭环判断: 已执行多流验证, 按评分/重传率选择最终配置"
        fi
    elif [[ "$multi_tested" == "true" && "$best_s" -ge 4 ]]; then
        log "  最优配置 ${C_BOLD}${best_s} 流${C_RESET}, 推荐应用层使用相同的并发数"
    fi
...
write_and_apply_sysctl() {
    ...
    if out=$(sysctl -p "$SYSCTL_FILE" 2>&1); then
        ok "sysctl 已写入 ($(echo "$out" | grep -c '=') 项)"
    else
        warn "部分 sysctl 应用失败 (可能是当前内核不支持的键):"
        echo "$out" | grep -E "error|cannot|unknown" >&2 || true
    fi

    # v2.9.5: 真正读回验证关键项是否生效 (不再只看 sysctl -p 的行数)
    local verified=0 total=0 k expect actual
    while IFS='=' read -r k expect; do
        k=$(echo "$k" | tr -d ' '); expect=$(echo "$expect" | tr -d ' ')
        [[ -z "$k" ]] && continue
        case "$k" in
            net.core.rmem_max|net.core.wmem_max|net.ipv4.tcp_congestion_control|net.core.default_qdisc)
                total=$((total+1))
                actual=$(sysctl -n "$k" 2>/dev/null | tr -d ' \t') || true
                if [[ "$actual" == "$expect" ]]; then
                    verified=$((verified+1)); vlog "  ✓ $k = $actual"
                else
                    warn "  ✗ $k 期望 ${expect} 实际 ${actual:-<读不到>} (未真正生效)"
                fi ;;
        esac
    done < <(grep -E '^(net\.core\.(rmem_max|wmem_max|default_qdisc)|net\.ipv4\.tcp_congestion_control)\s*=' "$SYSCTL_FILE")
    if (( total > 0 )); then
        if (( verified == total )); then
            ok "关键 sysctl 已读回验证生效 (${verified}/${total}: rmem/wmem_max, 拥塞算法, qdisc)"
        else
            warn "关键 sysctl 仅 ${verified}/${total} 生效 — 当前环境(容器/受限内核?)可能未真正应用, 调优效果有限"
        fi
    fi
}
...
backup_nic() {
    ...
    # 1) 我们会改的 offload 特性的当前 on/off (精确到短名, 便于还原)
    local offload_pairs="" s st
    for s in $NIC_OFFLOAD_SHORT; do
        st=$(_nic_feat_state "$iface" "$(_nic_offload_long "$s")")
        [[ -n "$st" ]] && offload_pairs="${offload_pairs}${s}=${st};"
    done
    ...
    ring=$(ethtool -g "$iface" 2>/dev/null | awk '
        ...
        incur && /^[A-Za-z]/ { gsub(":","",$1); printf "%s=%s;", $1, $2 }
    ')
    mtu_cur=$(cat "/sys/class/net/$iface/mtu" 2>/dev/null || echo "")

    # 3) RPS / RFS / XPS / IRQ 亲和当前值 (用于精确还原)
    local rps_json rpsflow_json xps_json irq_json
    rps_json=$(for q in /sys/class/net/"$iface"/queues/rx-*/rps_cpus; do
        [[ -f "$q" ]] && printf '%s\t%s\n' "$q" "$(cat "$q" 2>/dev/null)"
    done | jq -R 'split("\t") | {q:.[0], val:.[1]}' 2>/dev/null | jq -s '.' 2>/dev/null || echo '[]')
    rpsflow_json=$(for q in /sys/class/net/"$iface"/queues/rx-*/rps_flow_cnt; do
        [[ -f "$q" ]] && printf '%s\t%s\n' "$q" "$(cat "$q" 2>/dev/null)"
    done | jq -R 'split("\t") | {q:.[0], val:.[1]}' 2>/dev/null | jq -s '.' 2>/dev/null || echo '[]')
    xps_json=$(for q in /sys/class/net/"$iface"/queues/tx-*/xps_cpus; do
        [[ -f "$q" ]] && printf '%s\t%s\n' "$q" "$(cat "$q" 2>/dev/null)"
    done | jq -R 'split("\t") | {q:.[0], val:.[1]}' 2>/dev/null | jq -s '.' 2>/dev/null || echo '[]')
    local irqs
    irqs=$(awk -v p="$iface" '$NF ~ p {sub(":","",$1); print $1}' /proc/interrupts 2>/dev/null)
    irq_json=$(for irq in $irqs; do
        [[ -f "/proc/irq/$irq/smp_affinity" ]] && \
            printf '%s\t%s\n' "$irq" "$(cat "/proc/irq/$irq/smp_affinity" 2>/dev/null)"
    done | jq -R 'split("\t") | {irq:.[0], val:.[1]}' 2>/dev/null | jq -s '.' 2>/dev/null || echo '[]')

    # 4) irqbalance 当前是否在跑 (rollback 时据此决定要不要再起)
    local irqbal_active=false
    command -v systemctl >/dev/null 2>&1 && systemctl is-active irqbalance >/dev/null 2>&1 && irqbal_active=true

    jq -n --arg iface "$iface" --arg offload "$offload_pairs" --arg ring "$ring" \
          --arg mtu "$mtu_cur" --argjson rps "${rps_json:-[]}" \
          --argjson rpsflow "${rpsflow_json:-[]}" --argjson xps "${xps_json:-[]}" \
          --argjson irq "${irq_json:-[]}" --argjson irqbal "$irqbal_active" \
        '{iface:$iface, offload:$offload, ring:$ring, mtu:$mtu,
          rps:$rps, rps_flow:$rpsflow, xps:$xps, irq:$irq, irqbalance_active:$irqbal}' \
        > "$NIC_BACKUP"
}

# v2.9.0: 从 $NIC_BACKUP 精确还原 offload / ring / RPS / XPS / IRQ
restore_nic() {
    [[ -f "$NIC_BACKUP" ]] || return 0
    local iface; iface=$(jq -r '.iface // ""' "$NIC_BACKUP")
    [[ -z "$iface" ]] && return 0
    command -v ethtool >/dev/null 2>&1 || { warn "  无 ethtool, 跳过网卡 offload/ring 还原"; }

    # offload: 还原我们改过的每个特性到备份值
    local pairs; pairs=$(jq -r '.offload // ""' "$NIC_BACKUP")
    local p k v restored=0
    IFS=';' read -ra _arr <<<"$pairs"
    for p in "${_arr[@]}"; do
        [[ -z "$p" ]] && continue
        k="${p%%=*}"; v="${p#*=}"
        [[ "$v" == "on" || "$v" == "off" ]] || continue
        if command -v ethtool >/dev/null 2>&1 && ethtool -K "$iface" "$k" "$v" >/dev/null 2>&1; then
            restored=$((restored+1)); vlog "  offload 还原: $k=$v"
        fi
    done
    (( restored > 0 )) && ok "  offload 已还原 $restored 项 (含 LRO)"

    # ring buffer 还原
    local rxv txv
    rxv=$(jq -r '.ring // "" | capture("RX=(?<n>[0-9]+)").n // empty' "$NIC_BACKUP" 2>/dev/null)
    txv=$(jq -r '.ring // "" | capture("TX=(?<n>[0-9]+)").n // empty' "$NIC_BACKUP" 2>/dev/null)
    if command -v ethtool >/dev/null 2>&1 && [[ "$rxv" =~ ^[0-9]+$ ]]; then
        ethtool -G "$iface" rx "$rxv" ${txv:+tx "$txv"} >/dev/null 2>&1 \
            && ok "  ring buffer 已还原 rx=$rxv${txv:+ tx=$txv}" || true
    fi

    # RPS / RFS / XPS 还原
    local n=0
    while IFS=$'\t' read -r q val; do
        [[ -z "$q" || -z "$val" ]] && continue
        [[ -f "$q" ]] && echo "$val" > "$q" 2>/dev/null && n=$((n+1))
    done < <(jq -r '(.rps // [])[] | "\(.q)\t\(.val)"' "$NIC_BACKUP" 2>/dev/null)
    while IFS=$'\t' read -r q val; do
        [[ -z "$q" || -z "$val" ]] && continue
        [[ -f "$q" ]] && echo "$val" > "$q" 2>/dev/null && n=$((n+1))
    done < <(jq -r '(.rps_flow // [])[] | "\(.q)\t\(.val)"' "$NIC_BACKUP" 2>/dev/null)
    while IFS=$'\t' read -r q val; do
        [[ -z "$q" || -z "$val" ]] && continue
        [[ -f "$q" ]] && echo "$val" > "$q" 2>/dev/null && n=$((n+1))
    done < <(jq -r '(.xps // [])[] | "\(.q)\t\(.val)"' "$NIC_BACKUP" 2>/dev/null)
    (( n > 0 )) && ok "  RPS/RFS/XPS 已还原 $n 项"

    # IRQ 亲和还原
    local m=0
    while IFS=$'\t' read -r irq val; do
        [[ -z "$irq" || -z "$val" ]] && continue
        [[ -f "/proc/irq/$irq/smp_affinity" ]] && echo "$val" > "/proc/irq/$irq/smp_affinity" 2>/dev/null && m=$((m+1))
    done < <(jq -r '(.irq // [])[] | "\(.irq)\t\(.val)"' "$NIC_BACKUP" 2>/dev/null)
    (( m > 0 )) && ok "  IRQ 亲和已还原 $m 项"
}

apply_nic_tuning() {
    # v2.9.0: --no-nic-tune 时完全不动网卡硬件 (只 sysctl 生效)
    if [[ $NIC_TUNE -eq 0 ]]; then
        log "已指定 --no-nic-tune: 跳过全部网卡硬件优化 (offload/ring/RPS/XPS/IRQ), 仅 sysctl 生效"
        return 0
    fi
    ...
    if [[ $DRY_RUN -eq 1 ]]; then
        log "[dry-run] 将对 ${iface} 执行: 备份当前设置 → 开启 gro/tso/gso/sg/rx/tx/rxhash/rxvlan/txvlan$([[ $ENABLE_LRO -eq 1 ]] && echo /lro) → 调大 ring buffer"
        [[ $ENABLE_LRO -eq 0 ]] && log "[dry-run]   LRO 默认不开启 (需 --enable-lro)"
        [[ $STOP_IRQBALANCE -eq 1 ]] && log "[dry-run]   停止 irqbalance (rollback 时恢复)"
        [[ -n "$MTU" ]] && log "[dry-run]   设置 MTU=${MTU}"
        log "[dry-run]   配置 RPS/RFS/XPS + 分散 IRQ 亲和 (cpumask 逗号分组, 支持 >32 核)"
    ...
    # v2.9.0: LRO 默认不开 (转发/桥接/虚拟化/抓包场景会丢包或行为异常); 仅 --enable-lro 才开
    if [[ $ENABLE_LRO -eq 1 ]]; then
        if ethtool -K "$iface" lro on >/dev/null 2>&1; then
            ok "  LRO: on (--enable-lro; 注意转发/桥接场景不友好)"
        else
            vlog "  lro: skip (not supported)"
        fi
    else
        vlog "  lro: 跳过 (默认不开启, 需 --enable-lro)"
    fi
    ...
    # v2.9.0: cpumask 用逗号分组生成器, 64C/128C/192C 不再溢出
    local cpu_mask
    cpu_mask=$(cpu_mask_all "$cores")
    ...
    if [[ -x /usr/sbin/set_irq_affinity ]]; then
        ...
        if [[ -n "$irqs" ]]; then
            for irq in $irqs; do
                local mask; mask=$(cpu_mask_single "$((i % cores))" "$cores")
...
run_iperf3() {
    ...
    # v2.9.2: 不再吞掉 iperf3 stderr, 落到 ${out}.err 便于失败时给出真实原因
    if [[ ${#pre[@]} -gt 0 ]]; then
        "${pre[@]}" iperf3 "${args[@]}" > "$out" 2>"${out}.err"
    else
        iperf3 "${args[@]}" > "$out" 2>"${out}.err"
    fi
}

# v2.9.2: 从 iperf3 输出里提取失败原因 (-J 时错误在 JSON .error; 否则看 stderr)
iperf3_error_reason() {
    local out="$1" reason=""
    if [[ -s "$out" ]]; then
        reason=$(jq -r '.error // empty' "$out" 2>/dev/null)
    fi
    if [[ -z "$reason" && -s "${out}.err" ]]; then
        reason=$(grep -iE 'error|cannot|unable|refused|denied|no route|timed out|timeout' "${out}.err" 2>/dev/null | head -1) || true
        [[ -z "$reason" ]] && reason=$(head -1 "${out}.err" 2>/dev/null) || true
    fi
    echo "$reason"
}
...
bench_streams() {
    ...
    for ((i=1; i<=repeats; i++)); do
        ...
        if ! run_iperf3 "$server" "$port" "$streams" "$duration" "$tmp"; then
            # v2.9.2: 打印 iperf3 真实失败原因 (此前被吞掉, 无法定位)
            local _reason; _reason=$(iperf3_error_reason "$tmp")
            if [[ -n "$_reason" ]]; then
                warn "  第 $i 次测试失败: ${_reason}"
            else
                warn "  第 $i 次测试失败 (iperf3 无输出; 检查连通性/端口/防火墙)"
            fi
            rm -f "$tmp" "${tmp}.err"
            continue
        fi
...
# v2.9.0: 写入失败状态 JSON 并标记 progress, 供 cmd_bench 在全失败时 die
# (防止自动化系统只看退出码=0 就误判调优成功)
write_failure_state() {
    local reason="$1"
    if [[ -d "$STATE_DIR" ]]; then
        jq -n --arg ts "$(date -Iseconds)" --arg server "${SERVER:-}" \
              --argjson port "${PORT:-0}" --arg dir "${DIRECTION:-}" \
              --arg profile "${PROFILE:-}" --arg reason "$reason" \
            '{timestamp:$ts, server:$server, port:$port, direction:$dir,
              profile:$profile, status:"failed", error:$reason,
              single:null, best:null, all:[]}' > "$STATE_FILE" 2>/dev/null || true
        if [[ -f "$PROGRESS_FILE" ]]; then
            jq --arg ts "$(date -Iseconds)" --arg reason "$reason" \
               '.status="failed" | .finished=$ts | .error=$reason | .best=null' \
               "$PROGRESS_FILE" > "${PROGRESS_FILE}.tmp" 2>/dev/null \
               && mv "${PROGRESS_FILE}.tmp" "$PROGRESS_FILE" || true
        fi
    fi
}
...
find_optimal_parallel() {
    ...
    local target_bw="${6:-0}" start_p="${7:-1}"
    ...
    for streams in 1 2 4 8 16 32; do
        ...
        if (( target_bw > 0 )) && num_ge "$bw" "$target_bw"; then
            ADAPTIVE_STOP_REASON="parallel-target-reached-${streams}"
            log "  已达到 NIC 目标 (${bw} >= ${target_bw} Mbps), 停止继续增加并发"
            break
        fi
    ...
    # v2.9.0: 无任何成功样本时不再打印 "最优: null 流", 标记 progress 失败, 并 return 1
    # (即使被直接/单独调用, 函数本身也以非零返回, 杜绝调用方误判成功)
    if [[ "$best" == "null" || -z "$best" ]]; then
        warn "本轮多流寻优无任何成功样本 (best=null)"
        if [[ -f "$progress_file" ]]; then
            jq --arg ts "$(date -Iseconds)" \
               '.status="failed" | .finished=$ts | .best=null | .error="multi-stream: no successful sample"' \
               "$progress_file" > "${progress_file}.tmp" 2>/dev/null \
               && mv "${progress_file}.tmp" "$progress_file" || true
        fi
        jq -n --argjson all "$arr" '{all:$all, best:null}'
        return 1
    fi
...
_ssh_prefix() {
    ...
    elif [[ -n "$SSH_PASS" || -n "$SSH_PASS_FILE" ]]; then
        # v2.9.3: 密码模式下强制走密码认证, 否则 ssh 会先试 publickey 并报
        # "Permission denied (publickey)", 密码根本没被使用。关掉 pubkey、禁用 agent。
        base+=(-o PreferredAuthentications=password,keyboard-interactive
               -o PubkeyAuthentication=no -o IdentityAgent=none -o NumberOfPasswordPrompts=1)
    fi
    printf '%s\n' "${base[@]}"
}
...
remote_current_user() {
    remote_run 'id -un' | tr -d '\r' | head -1
}
...
remote_sudo_password() {
    ...
    if [[ -n "$SUDO_PASS_FILE" && -r "$SUDO_PASS_FILE" ]]; then
        head -n 1 "$SUDO_PASS_FILE"
        return 0
    fi
    if [[ -n "$SSH_PASS" ]]; then
        printf '%s' "$SSH_PASS"
    ...
    if [[ -n "$SSH_PASS_FILE" && -r "$SSH_PASS_FILE" ]]; then
        head -n 1 "$SSH_PASS_FILE"
        return 0
    fi
    return 1
}

remote_detect_sudo_mode() {
    [[ -z "$SSH_HOST" ]] && { REMOTE_SUDO_MODE="none"; return 1; }
    [[ "$REMOTE_SUDO_MODE" != "unknown" ]] && [[ "$REMOTE_SUDO_MODE" != "" ]] && return 0

    local user
    user=$(remote_current_user || true)
    if [[ "$user" == "root" || "$SSH_USER" == "root" ]]; then
        REMOTE_SUDO_MODE="root"
        return 0
    fi

    if remote_run 'sudo -n true' >/dev/null; then
        REMOTE_SUDO_MODE="sudo-nopass"
        return 0
    fi

    local sudo_pw
    sudo_pw=$(remote_sudo_password || true)
    if [[ -n "$sudo_pw" ]]; then
        if ssh_remote "printf '%s\n' $(printf '%q' "$sudo_pw") | sudo -S -p '' -k true" >/dev/null 2>&1; then
            REMOTE_SUDO_MODE="sudo-pass"
            return 0
        fi
    fi

    REMOTE_SUDO_MODE="none"
    return 1
}

remote_root_run() {
    ssh_reachable || return 1
    remote_detect_sudo_mode || true
    case "$REMOTE_SUDO_MODE" in
        root)
            ssh_remote "$@"
            ;;
        sudo-nopass)
            ssh_remote "sudo -n bash -lc $(printf '%q' "$*")"
            ;;
        sudo-pass)
            local sudo_pw
            sudo_pw=$(remote_sudo_password || true)
            [[ -n "$sudo_pw" ]] || { err "无法读取远端 sudo 密码"; return 1; }
            ssh_remote "printf '%s\n' $(printf '%q' "$sudo_pw") | sudo -S -p '' -k bash -lc $(printf '%q' "$*")"
            ;;
        *)
            err "远端用户 ${SSH_USER} 不是 root，且无法通过 sudo 自动提权"
            err "  请改用 root 登录，或为该用户配置 sudo（NOPASSWD 或与 SSH 密码一致）"
            return 1
            ;;
    esac
}

remote_iperf3_running() {
    remote_run "ss -ltn '( sport = :${PORT} )' | grep -q LISTEN" >/dev/null 2>&1
}

mark_remote_iperf3_state() {
    local started="$1"
    REMOTE_IPERF3_STARTED="$started"
    local script_path=""
    [[ -f "${STATE_DIR}/remote.json" ]] && script_path=$(jq -r '.script // ""' "${STATE_DIR}/remote.json" 2>/dev/null || echo "")
    jq -n --arg host "$SSH_HOST" --arg path "$script_path" --argjson port "$PORT" --argjson started "$started" \
        '{host:$host, script:$path, iperf3_port:$port, iperf3_started_by_tool:$started}' > "${STATE_DIR}/remote.json" 2>/dev/null || true
}

stop_remote_iperf3_if_started() {
    [[ -z "$SSH_HOST" ]] && return 0
    local started="${REMOTE_IPERF3_STARTED:-0}"
    if [[ -f "${STATE_DIR}/remote.json" ]]; then
        local rh
        rh=$(jq -r '.host // ""' "${STATE_DIR}/remote.json" 2>/dev/null)
        if [[ "$rh" == "$SSH_HOST" ]]; then
            started=$(jq -r '.iperf3_started_by_tool // 0' "${STATE_DIR}/remote.json" 2>/dev/null || echo 0)
        fi
    fi
    [[ "$started" == "1" || "$started" == "true" ]] || return 0

    log "回收本次临时拉起的远端 iperf3 -s (port ${PORT})"
    remote_root_run "pkill -f 'iperf3 -s -p ${PORT}' >/dev/null 2>&1 || true" >/dev/null 2>&1 || warn "远端 iperf3 -s 清理失败"
    mark_remote_iperf3_state 0
}

ensure_remote_iperf3_server() {
    log "检查远端 iperf3 服务端"
    if remote_iperf3_running; then
        mark_remote_iperf3_state 0
        ok "远端 iperf3 -s 已运行 (port ${PORT})"
        return 0
    fi

    warn "远端未发现 iperf3 -s, 尝试自动启动 (端口 ${PORT})"
    local start_cmd="
        set -e
        mkdir -p /var/log/iperf3-tune 2>/dev/null || true
        iperf3 -s -p ${PORT} -D >/var/log/iperf3-tune/iperf3-server.log 2>&1 || \
        nohup iperf3 -s -p ${PORT} >/var/log/iperf3-tune/iperf3-server.log 2>&1 < /dev/null &
    "
    remote_root_run "$start_cmd" >/dev/null 2>&1 || return 1

    local i
    for i in 1 2 3 4 5; do
        sleep 1
        if remote_iperf3_running; then
            mark_remote_iperf3_state 1
            ok "远端 iperf3 -s 已启动 (port ${PORT})"
            return 0
        fi
    done

    err "远端 iperf3 -s 启动失败或端口 ${PORT} 未进入监听"
    remote_run "tail -20 /var/log/iperf3-tune/iperf3-server.log 2>/dev/null || true" | sed 's/^/    | /' >&2 || true
    return 1
}
...
# ★ 关键修复: 远端缺依赖时, 自动自举依赖并在本次流程内拉起 iperf3 server
remote_tune() {
    ...
    if [[ $DRY_RUN -eq 1 ]]; then
        ...
        log "[dry-run]   自动处理远端依赖, 并确保远端 iperf3 -s 在端口 ${PORT} 运行"
        log "[dry-run] 预演结束, 未连接远端、未做任何修改"
        return 0
    fi

    local probe_err probe_rc remote_user
    ...
    remote_user=$(remote_current_user || true)
    remote_detect_sudo_mode || true
    case "$REMOTE_SUDO_MODE" in
        root)         log "远端登录用户: ${remote_user:-$SSH_USER} (root)" ;;
        sudo-nopass)  log "远端登录用户: ${remote_user:-$SSH_USER} (sudo 免密提权)" ;;
        sudo-pass)    log "远端登录用户: ${remote_user:-$SSH_USER} (sudo 使用 SSH 密码提权)" ;;
        *)
            err "远端 ${SSH_USER}@${SSH_HOST} 无法获得 root 权限，无法自动安装依赖或应用 sysctl 调优"
            err "  请使用 root 登录，或让该用户具备 sudo 权限"
            return 1
            ;;
    esac

    local remote_script
    remote_script=$(ssh_remote "mktemp /tmp/iperf3-tune.XXXXXX 2>/dev/null" 2>/dev/null | tr -d '\r' | head -1)
    if [[ -z "$remote_script" ]]; then
        remote_script="/tmp/iperf3-tune.$$.${RANDOM}.sh"
        warn "远端 mktemp 不可用, 回退到 ${remote_script}"
    fi
    jq -n --arg host "$SSH_HOST" --arg path "$remote_script" \
        '{host:$host, script:$path}' > "${STATE_DIR}/remote.json" 2>/dev/null || true
    ...
    ssh_remote "chmod 700 $remote_script" >/dev/null 2>&1 || true

    log "在远端执行 tune --profile=${PROFILE}"
    local rcmd
    rcmd="bash $remote_script tune --profile ${PROFILE} --congestion ${CONGESTION} --yes --auto-install"
    [[ -n "$MTU" ]] && rcmd="$rcmd --mtu ${MTU}"
    [[ $VERBOSE -eq 1 ]] && rcmd="$rcmd --verbose"
    [[ $STOP_IRQBALANCE -eq 0 ]] && rcmd="$rcmd --no-stop-irqbalance"
    [[ $NIC_TUNE -eq 0 ]] && rcmd="$rcmd --no-nic-tune"
    [[ $ENABLE_LRO -eq 1 ]] && rcmd="$rcmd --enable-lro"

    local remote_log="${LOG_DIR}/remote-tune.$(date +%s).log"
    local rc=0
    remote_root_run "$rcmd" 2>&1 | tee "$remote_log" >&2 || rc=${PIPESTATUS[0]}
    if (( rc == 0 )); then
    ...
    if ! ensure_remote_iperf3_server; then
        err "远端 iperf3 server 自启动失败"
        return 1
    fi
...
remote_rollback() {
    ...
    # v2.9.0: 从本地 state 读回 remote_tune 时记录的远端脚本路径
    local remote_script=""
    if [[ -f "${STATE_DIR}/remote.json" ]]; then
        local rh
        rh=$(jq -r '.host // ""' "${STATE_DIR}/remote.json" 2>/dev/null)
        [[ "$rh" == "$SSH_HOST" ]] && remote_script=$(jq -r '.script // ""' "${STATE_DIR}/remote.json" 2>/dev/null)
    fi
    if [[ -z "$remote_script" ]]; then
        warn "无远端脚本路径记录 (可能从未 remote_tune, 或在别处执行的), 跳过远端 rollback"
        return 0
    fi
    if ssh_remote "test -f $remote_script"; then
        ssh_remote "bash $remote_script rollback --yes" || warn "远端 rollback 返回非零"
        stop_remote_iperf3_if_started
        ssh_remote "rm -f $remote_script" >/dev/null 2>&1 || true
    else
        warn "远端 $remote_script 不存在, 跳过 (可能已被清理)"
    fi
}
...
tuning_impact_items() {
    ...
    # v2.9.4: 预先算好 LRO 提示, 避免在 items+=("...$(...)") 里嵌入会返回非零的命令替换
    # (ENABLE_LRO=0 时 $([[..]] && echo) 退出码 1, set -e 下会让 items+= 失败并触发 ERR trap)
    local _lro_note=""
    [[ ${ENABLE_LRO:-0} -eq 1 ]] && _lro_note="，含 LRO" || true
    if [[ $NIC_TUNE -eq 1 ]]; then
        [[ $STOP_IRQBALANCE -eq 1 ]] && items+=("停止 irqbalance 服务 (rollback 会恢复)") || true
        items+=("开启网卡 offload (gro/tso/gso/sg 等${_lro_note})，调大 ring buffer (rollback 会还原)")
        items+=("重设 RPS/RFS/XPS 与网卡 IRQ 亲和 (可能改变中断在 CPU 上的分布; rollback 会还原)")
    else
        items+=("已指定 --no-nic-tune: 不改动任何网卡硬件 (仅 sysctl 生效)")
    fi
    [[ -n "$MTU" ]] && items+=("把网卡 MTU 改为 ${MTU} (两端及中间链路都需支持, 否则可能丢包/不通)") || true
    [[ "$PROFILE" == "extreme" ]] && items+=("extreme 档: 关闭 rp_filter 反向路径过滤 + 1GB 缓冲, 对业务影响显著") || true
...
# v2.9.3: 快速连通性探测 — 跑一次 3s 的 iperf3, 成功返回 0; 失败打印真实原因并返回 1。
# 用最小参数 (不带 -w/-A/-Z), 先确认基本可达, 把"连不上"和"调参问题"区分开。
probe_iperf3_connectivity() {
    local tmp; tmp=$(mktemp /tmp/iperf3-tune-probe.XXXXXX.json)
    local -a a=(-c "$SERVER" -p "$PORT" -t 4 -O 1 --connect-timeout 5000 -J)
    [[ "$DIRECTION" == "reverse" ]] && a+=(-R)
    log "连通性探测: iperf3 -c ${SERVER} -p ${PORT} -t 4${DIRECTION:+ ($DIRECTION)} ... (顺带测真实链路速率)"
    if iperf3 "${a[@]}" > "$tmp" 2>"${tmp}.err"; then
        local got
        got=$(jq -r '(.end.sum_received.bits_per_second // .end.sum.bits_per_second // 0)' "$tmp" 2>/dev/null) || got=0
        rm -f "$tmp" "${tmp}.err"
        if [[ "$got" =~ ^[0-9.e+]+$ ]] && num_gt "$got" 0; then
            PROBE_MBPS=$(awk -v b="$got" 'BEGIN{printf "%.0f", b/1000000}')
            ok "连通性正常 (探测吞吐 ${PROBE_MBPS} Mbps)"
            return 0
        fi
        warn "连通性探测连上了但吞吐为 0 (server 或链路异常)"
        return 1
    fi
    local reason; reason=$(iperf3_error_reason "$tmp")
    rm -f "$tmp" "${tmp}.err"
    if [[ -n "$reason" ]]; then
        err "连通性探测失败: ${reason}"
    else
        err "连通性探测失败 (iperf3 无输出; 检查 server/端口/防火墙/IP)"
    fi
    return 1
}
...
cmd_bench() {
    ...
    # v2.9.4: 先做连通性探测 (3s), 它同时测出"真实链路吞吐", 用来定窗口比网卡标称(常 10G)准得多。
    # 连不上就立刻带真实原因失败, 不再浪费 1~2 分钟扫描全 0。
    probe_iperf3_connectivity || {
        write_failure_state "iperf3 连通性探测失败 (server=${SERVER}:${PORT}, direction=${DIRECTION})"
        die "无法连接 iperf3 server ${SERVER}:${PORT} (见上方真实原因)。常见: server 未跑 iperf3 -s / 端口或防火墙 / IP 写错 / -R 方向不支持"
    }

    # v2.9.4: 用户没显式给 --bandwidth 时, 用实测吞吐(留 2x 余量, 不超 NIC 标称)作为链路容量提示,
    # 避免按网卡 10G 标称把窗口算得过大, 在低内存/中速链路上把吞吐拖崩。
    if [[ ! "$BANDWIDTH" =~ ^[1-9][0-9]*$ ]] && num_gt "$PROBE_MBPS" 0; then
        local nic_mbps; nic_mbps=$(detect_nic_speed_mbps)
        BANDWIDTH=$(awk -v p="$PROBE_MBPS" -v n="$nic_mbps" 'BEGIN{
            v=p*2; if(v<100)v=100;            # 至少按 100Mbit 估, 给小链路留点窗口
            if(n>0 && v>n)v=n;                # 不超过网卡标称
            printf "%d", v
        }')
        log "链路容量(自动): 实测 ${PROBE_MBPS}Mbps → 按 ${BANDWIDTH}Mbit 估算窗口 (可用 --bandwidth 覆盖)"
    fi

    # v2.4.0: 解析单流绑核与 BDP 窗口 (此时 BANDWIDTH 已是实测值, 窗口更贴合真实链路)
    resolve_pin_and_window
    # v2.6.0: 零拷贝发送端严格报告
    report_zerocopy

    # v3.0.0: scan 模式先单流寻优, 未达到目标时自动继续多流闭环
    if [[ $SCAN_WINDOW -eq 1 || $SCAN_CONGESTION -eq 1 || $SCAN_CPU -eq 1 || $SCAN_ALL -eq 1 ]]; then
        local state; state=$(adaptive_scan_optimize)
        # v2.9.0: 最终确认测试无成功样本 → 写失败状态并非零退出
        if [[ "$(jq -r '.best.ok // false' <<<"$state" 2>/dev/null)" != "true" ]]; then
            echo "$state" > "$STATE_FILE" 2>/dev/null || true
            write_failure_state "单流寻优: 最终确认测试无成功样本 (server=${SERVER}:${PORT}, direction=${DIRECTION})"
            die "单流寻优最终确认测试失败 (无成功样本)。请检查: server 是否在 ${SERVER}:${PORT} 跑 iperf3 -s、防火墙/端口、方向(-R) 是否正确"
        fi
        echo "$state" > "$STATE_FILE"

        local nic_speed=0 iface_name=""
        iface_name=$(detect_primary_iface) || iface_name=""
        if [[ -n "$iface_name" && -r "/sys/class/net/$iface_name/speed" ]]; then
            nic_speed=$(cat "/sys/class/net/$iface_name/speed" 2>/dev/null || echo 0)
            [[ "$nic_speed" =~ ^-?[0-9]+$ ]] || nic_speed=0
        fi
        ...
        ok "闭环最优组合已写入 ${C_CYAN}${STATE_FILE}${C_RESET} (jq '.best,.optimization' 查看)"
    ...
    # v2.9.0: 单流失败时不再打印 "null Mbps", 明确提示失败
    local s_ok s_bw s_rr
    s_ok=$(jq -r '.ok // false' <<<"$single" 2>/dev/null)
    if [[ "$s_ok" == "true" ]]; then
        s_bw=$(jq -r '.bandwidth_mbps'    <<<"$single")
        s_rr=$(jq -r '.retrans_rate_pct'  <<<"$single")
        ok "单流: ${s_bw} Mbps, 重传率 ${s_rr}%"
    else
        warn "单流基准测试失败 (无成功样本), 继续尝试多流寻优"
    fi

    # 多流寻优 (内部从 1 流开始, 但我们已经测过单流, 仍然让算法走完, 因为它的复测是中位数)
    local optimal opt_rc
    optimal=$(find_optimal_parallel "$SERVER" "$PORT" "$DURATION" "$REPEATS" "$MAX_PARALLEL")
    opt_rc=$?   # v2.9.0: find_optimal_parallel 在无成功样本时返回非零

    # v2.9.0: 单流与多流都无成功样本 → 写失败状态并非零退出 (不让自动化误判成功)
    local single_ok best_present
    single_ok=$(jq -r '.ok // false' <<<"$single" 2>/dev/null)
    best_present=$(jq -r '(.best != null)' <<<"$optimal" 2>/dev/null)
    if [[ "$single_ok" != "true" && ( "$best_present" != "true" || "$opt_rc" -ne 0 ) ]]; then
        write_failure_state "所有 iperf3 测试均失败 (server=${SERVER}:${PORT}, direction=${DIRECTION})"
        die "所有 iperf3 测试均失败 (无成功样本)。请检查: server 是否在 ${SERVER}:${PORT} 跑 iperf3 -s、防火墙/端口、方向(-R) 是否正确"
    fi

    # 合成 state.json
    # v2.9.0: 若多流 best 为 null 但单流成功, best 回退到单流结果, 避免 null 向下游传播
    local state
    state=$(jq -n \
        --arg ts "$(date -Iseconds)" \
        --arg server "$SERVER" --argjson port "$PORT" \
        ...
        '{
            ...
            single: $single,
            all: ($opt.all // [$single]),
            best: ($opt.best // (if ($single.ok // false) then $single else null end))
        }')
    echo "$state" > "$STATE_FILE"
...
cmd_rollback() {
    ...
    if [[ -f "$NIC_BACKUP" ]]; then
        ...
        log "回退网卡 $iface 设置 (从 $NIC_BACKUP 精确还原)"
        if [[ -n "$mtu_old" && "$mtu_old" =~ ^[0-9]+$ ]]; then
            ip link set dev "$iface" mtu "$mtu_old" >/dev/null 2>&1 \
                && ok "  MTU 已还原为 $mtu_old" || warn "  MTU 还原失败"
        fi
        # v2.9.0: 完整还原 offload(含 LRO)/ring/RPS/XPS/IRQ
        restore_nic
    fi

    # v2.9.0: 仅当备份时 irqbalance 在跑才重新启动 (尊重原始状态)
    local irqbal_was
    irqbal_was=$(jq -r '.irqbalance_active // empty' "$NIC_BACKUP" 2>/dev/null)
    if command -v systemctl >/dev/null 2>&1; then
        if [[ "$irqbal_was" == "false" ]]; then
            vlog "  原本 irqbalance 未运行, 不重启"
        else
            systemctl start irqbalance >/dev/null 2>&1 && ok "  irqbalance 已恢复" || true
        fi
    fi
...
cmd_status() {
    ...
    if [[ -f "$STATE_FILE" ]]; then
        # v2.9.0: 失败状态单独提示, 不打印 "null 流"
        local st_status; st_status=$(jq -r '.status // "ok"' "$STATE_FILE" 2>/dev/null)
        if [[ "$st_status" == "failed" ]]; then
            warn "最近一次测试: ${C_RED}失败${C_RESET}"
            jq -r '
                "  时间:      \(.timestamp)",
                "  服务端:    \(.server):\(.port)",
                "  方向:      \(.direction)",
            ...
            jq -r '
                ...
                "  方向:      \(.direction)",
                "  profile:   \(.profile)",
                "  单流:      \(.single.bandwidth_mbps // "n/a") Mbps  (\(.single.retrans_rate_pct // "n/a")% 重传)",
                "  最优配置:  \(.best.streams // "n/a") 流  \(.best.bandwidth_mbps // "n/a") Mbps  (\(.best.retrans_rate_pct // "n/a")% 重传)"
            ' "$STATE_FILE" >&2
        fi
    else
...
cmd_stop() {
    ...
    rm -f "${STATE_DIR}/.detach-ssh.pw" "${STATE_DIR}/.detach-sudo.pw" 2>/dev/null || true
    ok "已停止"
}
...
#   - 命令行明文密码 (--ssh-pass / --sudo-pass) 与交互密码 (--ssh-ask-pass / --sudo-ask-pass)
#     一律转存为 600 文件, 后台进程改用 --*-pass-file, 避免在 ps / 进程列表里暴露明文
detach_to_background() {
    ...
    local ssh_pwfile=""
    local sudo_pwfile=""
    local captured_ssh_pass=""
    local captured_sudo_pass=""
    local skip_next=""
    local -a clean_args=()
    local a
    for a in "$@"; do
        if [[ -n "$skip_next" ]]; then
            case "$skip_next" in
                ssh)  captured_ssh_pass="$a" ;;
                sudo) captured_sudo_pass="$a" ;;
            esac
            skip_next=""
            continue
        fi
        case "$a" in
            --detach|-d)        continue ;;
            --ssh-pass)         skip_next="ssh"; continue ;;
            --sudo-pass)        skip_next="sudo"; continue ;;
            --ssh-ask-pass)     continue ;;
            --sudo-ask-pass)    continue ;;
            *)                  clean_args+=("$a") ;;
        esac
    done

    if [[ -n "$captured_ssh_pass" ]]; then
        ssh_pwfile="${STATE_DIR}/.detach-ssh.pw"
        ( umask 077; printf '%s' "$captured_ssh_pass" > "$ssh_pwfile" )
        chmod 600 "$ssh_pwfile"
        unset captured_ssh_pass
    elif [[ -n "${_SSH_PASS_TMP:-}" && -f "${_SSH_PASS_TMP}" ]]; then
        ssh_pwfile="${STATE_DIR}/.detach-ssh.pw"
        ( umask 077; cat "$_SSH_PASS_TMP" > "$ssh_pwfile" )
        chmod 600 "$ssh_pwfile"
    elif [[ -n "$SSH_PASS_FILE" ]]; then
        :
    fi
    if [[ -n "$ssh_pwfile" ]]; then
        clean_args+=(--ssh-pass-file "$ssh_pwfile")
    fi

    if [[ -n "$captured_sudo_pass" ]]; then
        sudo_pwfile="${STATE_DIR}/.detach-sudo.pw"
        ( umask 077; printf '%s' "$captured_sudo_pass" > "$sudo_pwfile" )
        chmod 600 "$sudo_pwfile"
        unset captured_sudo_pass
    elif [[ -n "${_SUDO_PASS_TMP:-}" && -f "${_SUDO_PASS_TMP}" ]]; then
        sudo_pwfile="${STATE_DIR}/.detach-sudo.pw"
        ( umask 077; cat "$_SUDO_PASS_TMP" > "$sudo_pwfile" )
        chmod 600 "$sudo_pwfile"
    elif [[ -n "$SUDO_PASS_FILE" ]]; then
        :
    fi
    if [[ -n "$sudo_pwfile" ]]; then
        clean_args+=(--sudo-pass-file "$sudo_pwfile")
    fi

    local env_prefix=""
    [[ -n "$ssh_pwfile" ]] && env_prefix+="IPERF3_TUNE_DETACH_PWFILE=${ssh_pwfile@Q} "
    [[ -n "$sudo_pwfile" ]] && env_prefix+="IPERF3_TUNE_DETACH_SUDO_PWFILE=${sudo_pwfile@Q} "
    setsid nohup bash -c "
        echo \$\$ > '$PID_FILE'
...
命令:
  ...
  tail                跟随后台任务日志
  stop                停止后台任务
  selftest            自检本版关键修复是否生效 (无需 iperf3/网络/root; 退出码 0=全过)
...
调优参数:
  ...
  --target-utilization PCT     scan 闭环目标 NIC 利用率 (默认 85; 0=不设目标)

单流优化参数 (v2.4.0 新增):
  --single                     仅测单流 (等价 --max-parallel 1)
  --window SIZE                手动 TCP 窗口, 如 64M; 不填则按 RTT×带宽 (2.0×BDP) 自动算
                               (会自动夹到内核 wmem/rmem 上限内, 不会设过大跑不起来)
  --window-max SIZE            自动窗口的绝对上限 (默认 96M; 单流高 RTT 打满 2G 用; 会再被内核/内存上限夹小)
  --bandwidth MBIT             链路容量提示 (Mbit/s) 用于自动算窗口; 不填用 NIC 标称速率
                               (注意: NIC 标称常是 10G/25G, 与实际宽带不符时建议显式指定)
...
SSH 参数 (远端同步调优):
  ...
  --sudo-ask-pass              交互式输入远端 sudo 密码 (SSH 密码与 sudo 密码不同场景用)
  --sudo-pass-file PATH        从文件读远端 sudo 密码 (权限须为 600/400)
  --sudo-pass PASS             远端 sudo 密码明文 (不推荐: 会进 history/ps/审计日志)
...
依赖与安全 (v2.7.0):
  ...
  --enable-lro                 额外开启 LRO (默认不开: 对转发/桥接/虚拟化/抓包不友好)
...
parse_args() {
    ...
    case "$CMD" in
        detect|tune|bench|optimize|rollback|status|watch|tail|stop|selftest|help|--help|-h|version|--version|-V)
            ;;
        *)
            err "未知命令: $CMD"
            usage
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            --target-utilization) TARGET_UTILIZATION="$2"; shift 2 ;;

            --single)          SINGLE_ONLY=1; shift ;;
            --window)          WINDOW="$2"; shift 2 ;;
            --window-max)      WINDOW_MAX="$2"; shift 2 ;;
            ...
            --scan)            SCAN_ALL=1; SCAN_REQUESTED=1; SINGLE_ONLY=1; shift ;;
            --scan-window)     SCAN_WINDOW=1; SCAN_REQUESTED=1; SINGLE_ONLY=1; shift ;;
            --scan-congestion) SCAN_CONGESTION=1; SCAN_REQUESTED=1; SINGLE_ONLY=1; shift ;;
            --scan-cpu)        SCAN_CPU=1; SCAN_REQUESTED=1; SINGLE_ONLY=1; shift ;;
            ...
            --sudo-pass-file)  SUDO_PASS_FILE="$2"; shift 2 ;;
            --sudo-ask-pass)   SUDO_ASK_PASS=1; shift ;;
            ...
            --no-nic-tune)     NIC_TUNE=0; shift ;;
            --enable-lro)      ENABLE_LRO=1; shift ;;
            -h|--help)         usage; exit 0 ;;
    ...
    if [[ $SUDO_ASK_PASS -eq 1 ]]; then
        [[ -n "$SUDO_PASS" ]] && warn "同时给了 --sudo-pass 和 --sudo-ask-pass, 以交互输入为准"
        local _sp1
        if [[ ! -t 0 ]]; then
            die "--sudo-ask-pass 需要交互终端 (tty)"
        fi
        read -rsp "请输入远端 sudo 密码: " _sp1; echo >&2
        [[ -n "$_sp1" ]] || die "sudo 密码为空"
        _SUDO_PASS_TMP="$(mktemp "${TMPDIR:-/tmp}/iperf3-tune.sudo-pw.XXXXXX")" || die "无法创建临时 sudo 密码文件"
        chmod 600 "$_SUDO_PASS_TMP"
        printf '%s' "$_sp1" > "$_SUDO_PASS_TMP"
        unset _sp1
        SUDO_PASS=""
        SUDO_PASS_FILE="$_SUDO_PASS_TMP"
    fi
    ...
    if [[ -n "$SUDO_PASS" ]]; then
        warn "检测到命令行明文 --sudo-pass: 可能被 shell history / ps / 审计系统记录"
        warn "  更安全的方式: --sudo-ask-pass (交互输入) / --sudo-pass-file (600 文件)"
    fi

    # 从 ssh-pass-file / sudo-pass-file 读密码文件前做权限校验
    ...
    if [[ -z "$SUDO_PASS" && -n "$SUDO_PASS_FILE" ]]; then
        [[ -r "$SUDO_PASS_FILE" ]] || die "无法读取 sudo 密码文件: $SUDO_PASS_FILE"
        local perm_sudo
        perm_sudo=$(stat -c "%a" "$SUDO_PASS_FILE" 2>/dev/null || echo "")
        if [[ "$perm_sudo" != "600" && "$perm_sudo" != "400" ]]; then
            warn "sudo 密码文件 $SUDO_PASS_FILE 权限 $perm_sudo 不安全, 建议 chmod 600"
        fi
    fi
    ...
    # v2.4.0: --single 仅测单流; scan 的 SINGLE_ONLY 仅用于复用单流参数阶段
    [[ $SINGLE_ONLY -eq 1 && $SCAN_REQUESTED -eq 0 ]] && MAX_PARALLEL=1
    if [[ -n "$MTU" ]]; then
    ...
    [[ "$TARGET_UTILIZATION" =~ ^[0-9]+([.][0-9]+)?$ ]] || die "--target-utilization 必须是 0 到 100 的数字"
    if num_lt "$TARGET_UTILIZATION" 0 || num_gt "$TARGET_UTILIZATION" 100; then
        die "--target-utilization 必须在 0 到 100 之间"
    fi
    # v2.5.0: 扫描参数校验
...
# v2.9.1: 自检命令 — 不需要 iperf3/网络/root, 验证本版关键修复是否生效。
# 退出码 0 = 全过; 非 0 = 有项不符合预期 (便于 CI 校验拿到的是正确版本)。
cmd_selftest() {
    local pass=0 fail=0
    _t() { # _t 描述 实际 期望
        if [[ "$2" == "$3" ]]; then pass=$((pass+1)); printf '  %s✓%s %-44s = %s\n' "${C_GREEN}" "${C_RESET}" "$1" "$2" >&2
        else fail=$((fail+1)); printf '  %s✗%s %-44s = %s (期望 %s)\n' "${C_RED}" "${C_RESET}" "$1" "$2" "$3" >&2; fi
    }
    log "${C_BOLD}========== iperf3-tune 自检 (v${VERSION}) ==========${C_RESET}"

    log "${C_BOLD}[#9] CPU mask 生成 (>32 核不溢出)${C_RESET}"
    _t "cpu_mask_all 64"        "$(cpu_mask_all 64)"        "ffffffff,ffffffff"
    _t "cpu_mask_all 128"       "$(cpu_mask_all 128)"       "ffffffff,ffffffff,ffffffff,ffffffff"
    _t "cpu_mask_all 48"        "$(cpu_mask_all 48)"        "ffff,ffffffff"
    _t "cpu_mask_single 63/64"  "$(cpu_mask_single 63 64)"  "80000000,0"
    _t "cpu_mask_single 32/64"  "$(cpu_mask_single 32 64)"  "1,0"

    log "${C_BOLD}[窗口] 夹取函数存在性${C_RESET}"
    _t "clamp_window_bytes 已定义"  "$(declare -F clamp_window_bytes >/dev/null && echo yes || echo no)" "yes"
    _t "to_bytes 非法输入→0"        "$(to_bytes 'xyz')"          "0"

    log "${C_BOLD}[#6] 全失败判定 (核心安全项)${C_RESET}"
    # 模拟 cmd_bench 的失败判定逻辑 (用内置假数据, 不调 iperf3)
    local single='{"streams":1,"ok":false}'
    local optimal='{"all":[{"ok":false}],"best":null}'
    local sok bp rc_emul=1
    sok=$(jq -r '.ok // false' <<<"$single" 2>/dev/null)
    bp=$(jq -r '(.best != null)' <<<"$optimal" 2>/dev/null)
    local would_die="no"
    [[ "$sok" != "true" && ( "$bp" != "true" || "$rc_emul" -ne 0 ) ]] && would_die="yes"
    _t "全失败 → 会 die 非零"      "$would_die"                 "yes"
    # 反例: 单流成功时不应 die
    single='{"streams":1,"ok":true,"bandwidth_mbps":940}'
    sok=$(jq -r '.ok // false' <<<"$single" 2>/dev/null); would_die="no"
    [[ "$sok" != "true" && ( "$bp" != "true" || "$rc_emul" -ne 0 ) ]] && would_die="yes"
    _t "单流成功 → 不 die"         "$would_die"                 "no"

    log "${C_BOLD}[#8] LRO 默认关 / 网卡可回退${C_RESET}"
    _t "ENABLE_LRO 默认值"         "$ENABLE_LRO"               "0"
    _t "NIC_TUNE 默认值"           "$NIC_TUNE"                 "1"
    _t "restore_nic 已定义"        "$(declare -F restore_nic >/dev/null && echo yes || echo no)" "yes"

    log "${C_BOLD}[#7] 拥塞算法切换可失败${C_RESET}"
    # set_sender_congestion 在 DRY_RUN 下 return 0; 这里只验证函数存在且非空实现
    _t "set_sender_congestion 含 return 1" \
       "$(declare -f set_sender_congestion | grep -c 'return 1' || true)" "2"

    log "${C_BOLD}[实测崩溃修复] measure_rtt 在不通时不崩脚本${C_RESET}"
    # 调用 measure_rtt 到一个不可达地址; 关键是脚本不因 grep 无匹配触发 errexit 崩掉
    local _rtt_out _rtt_rc
    _rtt_out=$(measure_rtt "203.0.113.250" 2>/dev/null); _rtt_rc=$?
    _t "measure_rtt 返回码=0(没崩)"   "$_rtt_rc"                  "0"
    _t "measure_rtt 输出形如 rtt|loss" "$([[ "$_rtt_out" == *"|"* ]] && echo yes || echo no)" "yes"
    _t "probe_iperf3_connectivity 已定义" "$(declare -F probe_iperf3_connectivity >/dev/null && echo yes || echo no)" "yes"
    _t "密码模式强制 password 认证" \
       "$(declare -f _ssh_prefix | grep -c 'PreferredAuthentications=password' || true)" "1"

    log "${C_BOLD}[崩栈修复] tuning_impact_items / 窗口实测定容${C_RESET}"
    local _ti_rc; tuning_impact_items >/dev/null 2>&1; _ti_rc=$?
    _t "tuning_impact_items 返回0(不崩)" "$_ti_rc" "0"
    _t "detect_nic_speed_mbps 已定义"   "$(declare -F detect_nic_speed_mbps >/dev/null && echo yes || echo no)" "yes"

    log "${C_BOLD}[日志可信度] sysctl 验证 / 多流结论${C_RESET}"
    _t "sysctl_writable 已定义(可靠试写)" "$(declare -F sysctl_writable >/dev/null && echo yes || echo no)" "yes"
    _t "preflight 用 sysctl_writable 判断" "$(declare -f preflight_check 2>/dev/null | grep -c 'sysctl_writable' || true)" "1"
    _t "analyze_link 区分是否测多流"      "$([[ $(declare -f analyze_link | grep -c 'multi_tested' || true) -ge 1 ]] && echo yes || echo no)" "yes"
    _t "不再断言'多流没有意义'"           "$(declare -f analyze_link | grep -c '多流没有意义' || true)" "0"

    echo "" >&2
    if (( fail == 0 )); then
        ok "自检全部通过 (${pass}/${pass}); 本版 v${VERSION} 关键修复均生效"
        return 0
    else
        err "自检有 ${fail} 项不符合预期 (通过 ${pass})"
        return 1
    fi
}
