...
run_iperf3() {
    ...
    local dir_flag=""
    [[ "$DIRECTION" == "reverse" ]] && dir_flag="-R"
    # -O 2 忽略前 2 秒 slow start, --connect-timeout 5s
    iperf3 -c "$server" -p "$port" -P "$streams" -t "$duration" -O 2 \
           --connect-timeout 5000 -J $dir_flag > "$out" 2>/dev/null
}
...
_ssh() {
    ...
    if [[ -n "$SSH_KEY" ]]; then
        ssh -o StrictHostKeyChecking=accept-new -o ConnectTimeout=10 \
            -i "$SSH_KEY" -p "$SSH_PORT" "${SSH_USER}@${SSH_HOST}" "$cmd"
    elif [[ -n "$SSH_PASS" ]]; then
        sshpass -p "$SSH_PASS" ssh -o StrictHostKeyChecking=accept-new -o ConnectTimeout=10 \
            -p "$SSH_PORT" "${SSH_USER}@${SSH_HOST}" "$cmd"
    else
        ssh -o StrictHostKeyChecking=accept-new -o ConnectTimeout=10 \
            -p "$SSH_PORT" "${SSH_USER}@${SSH_HOST}" "$cmd"
    fi
...
_scp() {
    local src="$1" dst="$2"
    if [[ -n "$SSH_KEY" ]]; then
        scp -o StrictHostKeyChecking=accept-new -i "$SSH_KEY" -P "$SSH_PORT" \
...
${C_BOLD}远端 SSH 选项${C_RESET}
  ...
  --ssh-key PATH            SSH 私钥路径 (推荐)
  --ssh-pass PASS           SSH 密码 (不推荐, 会进 ps/history)
...
parse_args() {
    ...
    case "$CMD" in
        ...
        detect|tune|bench|optimize|rollback|status) ;;
